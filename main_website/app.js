...
function createAgent(network) {
  ...
  return {
    ...
    async generate(tokens) {
      ...
      return { tokens: [best], confidence: "95.0" };
    },
    train() {}
  };
}
...
window.switchActiveModel = function(modelName) {
  ...
  if (modelName === 'vilotni_1') {
    ...
    if (heroTitle) heroTitle.textContent = "VilotNI 1.5 (Beta)";
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    window.networkInstance = new VilotNI15Network(window.backendInstance, dynamicVocabSize);
    window.networkInstance.vocab = window.vocabInstance;
    window.agentInstance = new RLEngine(window.networkInstance);
    ...
    const fallbackVocabSize = window.vocabInstance ? window.vocabInstance.vocabSize : 4638;
    const poolDim = 256;
    const embDim = 64;
