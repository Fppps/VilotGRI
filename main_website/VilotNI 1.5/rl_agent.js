/**
 * VilotNI 1.5 - RSL v28 Continuation + Lexical Root Families / True Neural Self-Conditioning / Type-Driven Query + Commitment / Memory RSI RLEngine
 *
 * V28 continuation + lexical-root cleanup: Deep mode uses a fresh neural forward on prompt + generated prefix, while legacy RSL grammar/controller forces are deliberately down-scaled so live neural evidence can actually move token choice. Each beam therefore conditions the next token on what that beam has actually said, while Event/Query frames, prompt-rooted provenance, RL policy, Human-Decodable Proposition grammar, pronoun/person/case agreement, Answer Slot Contract, grounded word associations, live candidate expansion, semantic routing, compact beam search, whole-answer verification, and adaptive generation length remain in control. RSL does not regenerate the answer in repeated passes.
 * Deep now gives each surviving beam a real prompt+prefix neural forward before
 * the next token decision. Legacy structural rules remain as constraints, but
 * their positive score force is intentionally reduced. Low mode keeps the
 * cheaper stochastic sampler.
 *
 * Goal:
 *   Deep = true neural autoregression + light RSL constraints + continuation-aware stopping + lexical-root diversity + beam search + final verifier.
 *   Low  = simple one-path stochastic autoregression.
 */
...
class RLEngine {
  constructor(network) {
    ...
    this.autoregressiveNeighborCache = new Map();
    ...
    this.reasoningConfig = {
      ...
      // V26 true neural self-conditioning.
      // The latent state remains useful as a cheap semantic prior, but Deep RSL
      // now also performs the SAME essential operation Low/RL performs: after
      // a beam emits a token, the next decision gets a fresh neural forward on
      // prompt + that beam's generated prefix. This is no longer an approximation
      // of autoregression. It is real neural self-conditioning.
      autoregressiveLatentEnabled: true,
      rslTrueNeuralSelfConditioningEnabled: true,
      rslLiveNeuralScoreWeight: 2.35, // retained for compatibility/diagnostics
      rslLiveNeuralCandidateLimit: 96,

      // V27 decoder cleanup. The old RSL decoder could contribute several
      // structural points per token while live neural logits were only allowed
      // to penalize alternatives. V27 reverses that asymmetry: live neural
      // evidence is a positive 0..1 signal, then legacy grammar/controller
      // terms act as light constraints instead of the main language generator.
      rslNeuralPositiveWeight: 5.60,
      rslPromptNeuralPositiveWeight: 3.60,
      rslNeuralRelativeTemperature: 2.00,
      rslLegacyTransitionScale: 0.28,
      rslLegacyRoleScale: 0.24,
      rslLegacyLookaheadScale: 0.24,
      rslHumanTransitionScale: 0.42,
      rslPromptTransitionScale: 0.18,
      rslImportLegacyPersistence: false,
      rslDecoderDiagnosticsEnabled: true,

      // V28 continuation control. Semantic resolution now PERMITS stopping; it
      // does not immediately force a 2-6 token answer. A shape-aware soft
      // minimum keeps the live neural prefix active long enough to develop a
      // useful clause, while targetLength remains the absolute ceiling.
      rslContinuationSoftMinFraction: 0.58,
      rslShortStopMinLength: 6,
      rslYesNoStopMinLength: 8,
      rslProseStopMinLength: 8,
      rslTemporalStopMinLength: 6,
      rslPostPredicateSoftLimit: 8,
      rslPostPredicateExtraPenalty: 0.55,

      // V28 lexical-root family veto. Existing morphological-family checks
      // catch ordinary inflection. This second layer also catches derivational
      // relatives such as explain/explanation/explainable and
      // accelerate/acceleration/accelerator within the SAME clause.
      lexicalRootFamilyEnabled: true,
      lexicalRootMinimumLength: 4,
      lexicalRootFuzzyMinimumLength: 5,
      lexicalRootFuzzyEditDistance: 1,

      // The latent path now supplements the live neural forward instead of
      // pretending to replace it. These weights remain useful for beam-local
      // semantic momentum and knowledge-neighbor routing.
      autoregressiveLatentCandidateWeight: 3.80,
      autoregressiveLatentMaxBonus: 4.80,
      autoregressiveLatentContentTokenWeight: 0.30,
      autoregressiveLatentFunctionTokenWeight: 0.12,
      autoregressiveLatentPromptWeight: 0.08,
      autoregressiveLatentGuideWeight: 0.06,
      autoregressiveFrozenLogitFloor: 0.30,
      autoregressiveFrozenLogitDecay: 0.18,
      autoregressiveLiveProjectionWeight: 2.20,
      autoregressiveLiveDeltaWeight: 4.20,
      autoregressiveTransitionWeight: 2.40,
      autoregressiveNeighborLimit: 24,
      autoregressiveKnowledgeNeighborLimit: 24,
      autoregressiveCandidateExpansionLimit: 160,
      autoregressiveLivePoolLimit: 40,
    ...
    // V28 deliberately starts a fresh RSL continuation/lexical-learning namespace.
    // Existing V27/V23/V22 localStorage is left untouched, but it is NOT imported
    // unless rslImportLegacyPersistence is explicitly enabled for an experiment.
    this.persistenceKey = "vilotni15_rsl_continuation_v28";
    this.legacyPersistenceKey = "vilotni15_rsl_decoder_v27";
  ...
  initNormalizedEmbeddings() {
    ...
    // Any cached nearest-neighbor list was calculated from the old embedding
    // geometry. Drop it whenever the network changes its embeddings.
    this.autoregressiveNeighborCache?.clear?.();
  }
  ...
  lexicalRootCandidates(tokenOrWord) {
    const vocab = this.network?.vocab;
    const tokenId = Number.isInteger(tokenOrWord)
      ? tokenOrWord
      : null;
    const surface = tokenId !== null
      ? this.tokenWord(tokenId)
      : String(tokenOrWord || "");

    const seeds = new Set();
    const addSeed = value => {
      const normalized = this.normalizeLexicalRootWord(value);
    ...
    if (tokenId !== null) {
      const grammar = vocab?.getGrammar?.(tokenId) || null;
      addSeed(grammar?.lemma);
      addSeed(grammar?.root);
      addSeed(grammar?.base);
    }
    ...
    const minLen = Math.max(
      3,
      this.reasoningConfig?.lexicalRootMinimumLength || 4
    );
    ...
    while (queue.length) {
      ...
      seen.add(form);
      if (form.length >= minLen) roots.add(form);

      // Productive inflectional variants.
      if (form.endsWith("ies") && form.length > 5) push(form.slice(0, -3) + "y");
      if (form.endsWith("ied") && form.length > 5) push(form.slice(0, -3) + "y");
      if (form.endsWith("ing") && form.length > 6) {
        const base = form.slice(0, -3);
        push(base);
        push(base + "e");
        if (base.length >= 4 && base.at(-1) === base.at(-2)) push(base.slice(0, -1));
      }
      if (form.endsWith("ed") && form.length > 5) {
        const base = form.slice(0, -2);
        push(base);
        push(base + "e");
        if (base.length >= 4 && base.at(-1) === base.at(-2)) push(base.slice(0, -1));
      }
      if (form.endsWith("es") && form.length > 5) push(form.slice(0, -2));
      if (form.endsWith("s") && !form.endsWith("ss") && form.length > 4) push(form.slice(0, -1));

      // Productive derivational families. These are deliberately conservative:
      // they create candidate roots, and sameLexicalRootFamily() still requires
      // exact/fuzzy root agreement before a candidate is vetoed.
      const derivational = [
        ["abilities", ""], ["ability", ""], ["ably", ""], ["able", ""],
        ["ibilities", ""], ["ibility", ""], ["ibly", ""], ["ible", ""],
        ["izations", "ize"], ["ization", "ize"], ["isers", "ise"], ["iser", "ise"],
        ["izers", "ize"], ["izer", "ize"],
        ["ations", ""], ["ation", ""], ["itions", ""], ["ition", ""],
        ["tions", ""], ["tion", ""], ["sions", ""], ["sion", ""],
        ["ments", ""], ["ment", ""], ["nesses", ""], ["ness", ""],
        ["ities", ""], ["ity", ""], ["ances", ""], ["ance", ""],
        ["ences", ""], ["ence", ""], ["ives", ""], ["ive", ""],
        ["ously", ""], ["ous", ""], ["ally", ""], ["al", ""],
        ["ically", "ic"], ["ical", "ic"], ["ics", "ic"],
        ["ifiers", "ify"], ["ifier", "ify"], ["ifies", "ify"], ["ified", "ify"],
        ["ers", ""], ["er", ""], ["ors", ""], ["or", ""],
        ["ly", ""]
      ];
      ...
      for (const [suffix, replacement] of derivational) {
        if (
          ...
          form.length - suffix.length + replacement.length >= minLen
        ) {
          let base = form.slice(0, -suffix.length) + replacement;
          push(base);
          // accelerate -> accelerat, calculate -> calculat. This makes the
          // corresponding -ion/-or forms share a stable technical root.
          if (base.endsWith("e") && base.length > minLen) push(base.slice(0, -1));
          if (base.length >= 4 && base.at(-1) === base.at(-2)) push(base.slice(0, -1));
        }
      }

      // Also expose a silent-e stem for direct verb forms.
      if (form.endsWith("e") && form.length > minLen + 1) {
        push(form.slice(0, -1));
      }
    }
  ...
  lexicalRootEditDistanceAtMost(a, b, limit = 1) {
    a = String(a || "");
    b = String(b || "");
    if (a === b) return true;
    if (Math.abs(a.length - b.length) > limit) return false;

    // Small bounded Levenshtein. Roots are short and limit is normally 1, so
    // this exits quickly without allocating a full matrix.
  ...
  sameLexicalRootFamily(aToken, bToken) {
    if (!this.reasoningConfig?.lexicalRootFamilyEnabled) return false;
    if (!Number.isInteger(aToken) || !Number.isInteger(bToken)) return false;
    if (this.canonicalTokenId(aToken) === this.canonicalTokenId(bToken)) return true;

    const vocab = this.network?.vocab;
    if (
      typeof vocab?.sameMorphologicalFamily === "function" &&
      vocab.sameMorphologicalFamily(aToken, bToken)
    ) {
      return true;
    }

    const aRoots = this.lexicalRootCandidates(aToken);
    const bRoots = this.lexicalRootCandidates(bToken);
    const exact = new Set(aRoots);
    for (const root of bRoots) {
      if (exact.has(root)) return true;
    }

    const fuzzyMin = Math.max(
      5,
      this.reasoningConfig?.lexicalRootFuzzyMinimumLength || 5
    );
    const editLimit = Math.max(
      0,
      this.reasoningConfig?.lexicalRootFuzzyEditDistance ?? 1
    );

    for (const a of aRoots) {
      if (a.length < fuzzyMin) continue;
      for (const b of bRoots) {
        if (b.length < fuzzyMin) continue;
        const minLength = Math.min(a.length, b.length);
        if (Math.abs(a.length - b.length) > 2) continue;

        // explanation -> explan, explain -> explain differs by one vowel.
        if (this.lexicalRootEditDistanceAtMost(a, b, editLimit)) return true;

        // Very-near prefix variants cover a small set of orthographic
        // derivations without treating broad shared prefixes as a family.
        let common = 0;
        while (common < minLength && a[common] === b[common]) common++;
        if (minLength >= 6 && common >= minLength - 1) return true;
      }
    }
  ...
  loadPatternMemory() {
    ...
    try {
      ...
      if (
        !raw &&
        this.reasoningConfig?.rslImportLegacyPersistence === true &&
  ...
  /**
   * V25 autoregressive latent self-conditioning.
   *
   * Deep mode intentionally does NOT call the 15-layer neural network after
   * every token. Instead, each beam owns a tiny recurrent semantic state. The
   * state begins at the prompt's latent-reasoning centroid and is updated by
   * the embedding of every token that beam emits.
   *
   * This is beam-local. Competing answer paths therefore stop sharing one
   * frozen semantic snapshot as soon as their generated prefixes diverge.
   */
  createAutoregressiveLatentState(contextModel) {
    if (
      !this.reasoningConfig?.autoregressiveLatentEnabled ||
      !this.normalizedEmbeddings
    ) {
      return null;
    }

    let seed = contextModel?.reasoningCentroid || null;
    ...
    if (!seed) {
      ...
      if (topic && intent) {
        seed = this.blendVectors(topic, intent, 0.58, 0.42);
      } else {
        seed = topic || intent || null;
      }
    }
  ...
  cloneAutoregressiveLatentState(state) {
    if (!state?.vector) return state || null;
    return {
      vector: Float32Array.from(state.vector),
      updates: state.updates || 0,
      stability: Number.isFinite(state.stability) ? state.stability : 1.0,
      lastToken: Number.isInteger(state.lastToken) ? state.lastToken : null,
      transitionScores: new Map(state.transitionScores || [])
    };
  }

  advanceAutoregressiveLatentState(
    previousState,
    tokenId,
    contextModel
  ) {
    if (!this.reasoningConfig?.autoregressiveLatentEnabled) {
      return previousState || null;
    }

    const state = previousState || this.createAutoregressiveLatentState(contextModel);
    if (!state?.vector || !Number.isInteger(tokenId)) return state;

    const embDim = this.network.embDim;
    const vocabSize = this.network.vocabSize;
    if (!embDim || tokenId < 0 || tokenId >= vocabSize) return state;

    const type = this.getCanonicalType(tokenId);
    const isPunct = type === "Punct";
    const isContent = this.isContentType(type);

    const tokenWeight = isPunct
      ? 0.0
      : isContent
        ? this.reasoningConfig.autoregressiveLatentContentTokenWeight
        : this.reasoningConfig.autoregressiveLatentFunctionTokenWeight;

    const promptWeight = isPunct
      ? 0.15
      : this.reasoningConfig.autoregressiveLatentPromptWeight;

    const guideWeight = isPunct
      ? 0.0
      : this.reasoningConfig.autoregressiveLatentGuideWeight;

    const retention = Math.max(
      0.45,
      1.0 - tokenWeight - promptWeight - guideWeight
    );

    const promptSeed =
      contextModel?.reasoningCentroid ||
      contextModel?.centroid ||
      state.vector;

    const actionTypes = new Set(["Verb", "Aux", "Modal", "Adv"]);
    const guide = actionTypes.has(type)
      ? (contextModel?.intentCentroid || promptSeed)
      : (contextModel?.topicCentroid || promptSeed);

    const tokenOff = tokenId * embDim;
    const next = new Float32Array(embDim);

    for (let d = 0; d < embDim; d++) {
      next[d] =
        state.vector[d] * retention +
        this.normalizedEmbeddings[tokenOff + d] * tokenWeight +
        (promptSeed?.[d] || 0) * promptWeight +
        (guide?.[d] || 0) * guideWeight;
    }
    ...
    let dot = 0.0;
    for (let d = 0; d < embDim; d++) {
      dot += state.vector[d] * next[d];
    }

    // Build a compact token-conditioned continuation prior. This is deliberately
    // beam-local: two beams that emit different words immediately acquire
    // different continuation maps even though they share the same original
    // prompt forward.
    const transitionScores = new Map();
    const vocab = this.network.vocab;
    const limit = Math.max(8, this.reasoningConfig.autoregressiveKnowledgeNeighborLimit || 24);
    const related = typeof vocab?.getKnowledgeNeighbors === "function"
      ? vocab.getKnowledgeNeighbors(tokenId, limit)
      : typeof vocab?.getAssociations === "function"
        ? vocab.getAssociations(tokenId, limit)
        : [];

    for (const item of related || []) {
      if (!Number.isInteger(item?.id)) continue;
      const strength = Number(item.score ?? item.weight ?? 0);
      if (!Number.isFinite(strength)) continue;
      transitionScores.set(item.id, Math.max(0.0, Math.min(1.0, strength)));
    }

    // Preserve a little momentum from the previous step instead of replacing
    // the continuation prior outright.
    for (const [id, strength] of state.transitionScores || []) {
      const carried = Math.max(0.0, Math.min(1.0, Number(strength) || 0)) * 0.32;
      if (carried <= 0.02) continue;
      transitionScores.set(id, Math.max(transitionScores.get(id) || 0, carried));
    }
  ...
  getAutoregressiveLatentBonus(
    ...
    if (
      !this.reasoningConfig?.autoregressiveLatentEnabled ||
      !liveState?.vector ||
      (liveState.updates || 0) <= 0
    ) {
      return 0.0;
    }

    const type = tokenType || this.getCanonicalType(candidateToken);
    if (type === "Punct" || type === "Other") return 0.0;

    const live = this.dotTokenVector(candidateToken, liveState.vector);
    const frozen = Number.isFinite(contextModel?.reasoningScores?.[candidateToken])
      ? contextModel.reasoningScores[candidateToken]
      : 0.0;

    // The delta term is the important part: it rewards candidates that became
    // more appropriate because of the generated prefix, rather than simply
    // counting the same prompt semantics twice.
    const delta = live - frozen;

    let typeWeight = 1.0;
    if (["Aux", "Modal", "Prep", "Conj"].includes(type)) typeWeight = 0.48;
    else if (["Det", "Pronoun"].includes(type)) typeWeight = 0.34;

    const updates = Math.max(1, liveState.updates || 1);
    const maturity = Math.min(1.0, 0.28 + updates * 0.18);
    const transition = Math.max(
      0.0,
      Math.min(1.0, liveState.transitionScores?.get(candidateToken) || 0.0)
    );

    const projectionWeight =
      this.reasoningConfig.autoregressiveLiveProjectionWeight || 2.20;
    const deltaWeight =
      this.reasoningConfig.autoregressiveLiveDeltaWeight || 4.20;
    const transitionWeight =
      this.reasoningConfig.autoregressiveTransitionWeight || 2.40;

    // Unlike V24, the live projection is not a small semantic tie-breaker.
    // Delta measures how much the candidate became more appropriate AFTER the
    // model generated its prefix. Transition support provides a direct local
    // continuation prior from the token that was actually emitted.
    const raw =
      (
        live * projectionWeight +
        delta * deltaWeight +
        transition * transitionWeight
      ) *
      typeWeight *
      maturity *
      this.reasoningConfig.autoregressiveLatentCandidateWeight;

    const cap = this.reasoningConfig.autoregressiveLatentMaxBonus;
    return Math.max(-cap, Math.min(cap, raw));
  }

  getAutoregressiveEmbeddingNeighbors(tokenId, limit = null) {
    if (!Number.isInteger(tokenId) || !this.normalizedEmbeddings) return [];

    const wanted = Math.max(
      1,
      Math.round(limit || this.reasoningConfig.autoregressiveNeighborLimit || 18)
    );

    const revision = Number.isFinite(this.normalizedEmbeddingsRevision)
      ? this.normalizedEmbeddingsRevision
      : 0;
    const cacheKey = `${revision}:${tokenId}:${wanted}`;
    const cached = this.autoregressiveNeighborCache?.get(cacheKey);
    if (cached) return cached;

    const vocab = this.network.vocab;
    const vocabSize = this.network.vocabSize;
    const embDim = this.network.embDim;
    const sourceOff = tokenId * embDim;
    const best = [];

    for (let i = 0; i < vocabSize; i++) {
      if (i === tokenId || !vocab?.hasToken?.(i)) continue;
      ...
      for (let d = 0; d < embDim; d++) {
        sim +=
          this.normalizedEmbeddings[sourceOff + d] *
          this.normalizedEmbeddings[off + d];
      }

      if (best.length < wanted) {
        best.push({ id: i, score: sim });
        best.sort((a, b) => b.score - a.score);
      } else if (sim > best[best.length - 1].score) {
        best[best.length - 1] = { id: i, score: sim };
        best.sort((a, b) => b.score - a.score);
      }
    }

    const result = best.map(item => item.id);
    this.autoregressiveNeighborCache?.set(cacheKey, result);
    return result;
  }

  buildAutoregressiveCandidatePool(
    basePool,
    generated,
    contextModel,
    liveState = null,
    ...
    for (let i = generated.length - 1; i >= 0; i--) {
      ...
      if (this.isContentType(this.getCanonicalType(token))) {
        lastContentToken = token;
        break;
      }
    }

    if (!Number.isInteger(lastContentToken)) return basePool;

    const vocab = this.network.vocab;
    const result = new Set(basePool || []);
    const baseSize = result.size;
    const maxAdds = Math.max(
      0,
      this.reasoningConfig.autoregressiveCandidateExpansionLimit || 48
    );

    const tryAdd = token => {
      if (result.size - baseSize >= maxAdds) return false;
      if (!Number.isInteger(token) || !vocab?.hasToken?.(token)) return true;
      result.add(token);
      return result.size - baseSize < maxAdds;
    };

    // V26: reserve the expansion budget FIRST for the current neural prefix.
    // This is the key difference from V25. A token can enter RSL because the
    // actual 15-layer network likes it after reading what RSL has already said,
    // even if that token was absent from the original prompt-derived pool.
    if (liveRawLogits && liveRawLogits.length === this.network.vocabSize) {
      const neuralLimit = Math.max(16, this.reasoningConfig.rslLiveNeuralCandidateLimit || 96);
      const topNeural = [];
      for (let token = 0; token < liveRawLogits.length; token++) {
        if (!vocab?.hasToken?.(token)) continue;
        const type = this.getCanonicalType(token);
        if (type === "Other") continue;
        const value = liveRawLogits[token];
        if (!Number.isFinite(value)) continue;
        if (topNeural.length < neuralLimit) {
          topNeural.push({ token, value });
          topNeural.sort((a, b) => b.value - a.value);
        } else if (value > topNeural[topNeural.length - 1].value) {
          topNeural[topNeural.length - 1] = { token, value };
          topNeural.sort((a, b) => b.value - a.value);
        }
      }
      for (const item of topNeural) {
        if (!tryAdd(item.token)) break;
      }
    }

    const knowledgeLimit =
      this.reasoningConfig.autoregressiveKnowledgeNeighborLimit || 16;

    const related = typeof vocab?.getKnowledgeNeighbors === "function"
      ? vocab.getKnowledgeNeighbors(lastContentToken, knowledgeLimit)
      : typeof vocab?.getAssociations === "function"
        ? vocab.getAssociations(lastContentToken, knowledgeLimit)
        : [];

    for (const item of related || []) {
      if (!tryAdd(item?.id)) break;
    }

    if (result.size - baseSize < maxAdds) {
      for (const token of this.getAutoregressiveEmbeddingNeighbors(lastContentToken)) {
        if (!tryAdd(token)) break;
      }
    }

    // V25: project the CURRENT beam latent state back across the vocabulary.
    // This is the cheap autoregressive substitute for calling the 15-layer
    // network again. V24 only reranked the old pool, which meant a strong
    // frozen pool could still produce the exact same answer.
    if (liveState?.vector && (liveState.updates || 0) > 0) {
      const liveLimit = Math.max(8, this.reasoningConfig.autoregressiveLivePoolLimit || 40);
      const bestLive = [];
      const vocabSize = this.network.vocabSize;
      ...
      for (let token = 0; token < vocabSize; token++) {
        if (!vocab?.hasToken?.(token)) continue;
        const type = this.getCanonicalType(token);
        if (type === "Other" || type === "Punct") continue;

        const live = this.dotTokenVector(token, liveState.vector);
        const frozen = Number.isFinite(contextModel?.reasoningScores?.[token])
          ? contextModel.reasoningScores[token]
          : 0.0;
        const delta = live - frozen;
        const transition = liveState.transitionScores?.get(token) || 0.0;
        const rank = live * 0.62 + delta * 0.78 + transition * 0.55;

        if (bestLive.length < liveLimit) {
          bestLive.push({ token, rank });
          bestLive.sort((a, b) => b.rank - a.rank);
        } else if (rank > bestLive[bestLive.length - 1].rank) {
          bestLive[bestLive.length - 1] = { token, rank };
          bestLive.sort((a, b) => b.rank - a.rank);
        }
      }

      for (const item of bestLive) {
        if (!tryAdd(item.token)) break;
      }
    }

    // Keep the prompt's required anchors available even when the base pool was
    // constructed by an older/recovered round.
  ...
  materializeBestPrefix(
    ...
    if (
      ...
      !bestTokens.length ||
      bestTokens.length < Math.max(0, minimumLength) ||
      bestTokens.length >=
    ...
    return {
      ...
      state:
        beam.bestPrefixState
          ...
          : beam.state,
      latentState:
        beam.bestPrefixLatentState
          ? this.cloneAutoregressiveLatentState(
              beam.bestPrefixLatentState
            )
          : beam.latentState,
      sentenceCount:
  ...
  getRSLMinimumContinuationLength(
    targetLength,
    contextModel
  ) {
    const shape =
    ...
    let shapeFloor =
      this.reasoningConfig
        .rslProseStopMinLength;

    if (shape === "short") {
      shapeFloor =
        this.reasoningConfig
          .rslShortStopMinLength;
    } else if (shape === "yes_no") {
      shapeFloor =
        this.reasoningConfig
          .rslYesNoStopMinLength;
    } else if (shape === "temporal") {
      shapeFloor =
        this.reasoningConfig
          .rslTemporalStopMinLength;
    }
    ...
    const fractionFloor =
      Math.ceil(
        ...
        this.reasoningConfig
          .rslContinuationSoftMinFraction
      );

    // The floor can never exceed the actual target ceiling.
    return Math.max(
      1,
      Math.min(
        Math.max(1, targetLength),
  ...
  shouldStopDeepAnswer(
    ...
    const minimumContinuationLength =
      this.getRSLMinimumContinuationLength(
        targetLength,
        contextModel
      );
    ...
    // Resolution now PERMITS stopping only after the shape-aware continuation
    // floor. targetLength remains the hard ceiling, while the neural prefix gets
    // enough room to develop beyond the old 2-6-token stop wall.
    return ceilingReached;
  }
  ...
  getHumanTransitionAssessment(
    ...
    if (
      ...
      (sentenceState.postPredicateContentCount || 0) >=
        this.reasoningConfig.rslPostPredicateSoftLimit &&
      ![
        "Punct", "Prep", "Conj"
      ].includes(curType)
    ) {
      // V28: this used to be a HARD veto at five post-predicate content words,
      // which mechanically capped otherwise healthy neural continuations.
      // Long clauses now remain legal but become progressively less attractive.
      const excess =
        (sentenceState.postPredicateContentCount || 0) -
        this.reasoningConfig.rslPostPredicateSoftLimit +
        1;
      score -=
        Math.min(
          ...
          excess *
            this.reasoningConfig
              .rslPostPredicateExtraPenalty
        );
      quality = Math.min(
        quality,
        Math.max(0.42, 0.78 - excess * 0.06)
      );
    }
  ...
  buildScores(
    ...
    targetLength,
    reuseScores = null,
    liveLatentState = null,
    candidatePoolOverride = null,
    rawLogitsAreLivePrefix = false,
    scoreBreakdownSink = null
  ) {
    const vocabSize = this.network.vocabSize;
    const vocab = this.network.vocab;

    // V27 guard: this argument was easy to mis-wire while V24-V26 were being
    // patched incrementally. Fail closed to a Boolean instead of allowing a
    // truthy candidate-pool object to masquerade as the live-prefix flag.
    rawLogitsAreLivePrefix = rawLogitsAreLivePrefix === true;

    const decoderDiagnosticsEnabled = Boolean(
      deepMode &&
      this.reasoningConfig?.rslDecoderDiagnosticsEnabled &&
      scoreBreakdownSink &&
      typeof scoreBreakdownSink.set === "function"
    );
    ...
    const candidateIds =
      deepMode && candidatePoolOverride?.length
    ...
    // Frozen prompt logits are useful at token 0, but they must not remain the
    // dominant neural signal forever. Relative logits preserve the original
    // ranking while allowing live prefix evidence to take progressively more
    // control of continuation.
    let deepMaxRaw = -Infinity;
    if (deepMode) {
      for (let scanIndex = 0; scanIndex < scanCount; scanIndex++) {
        const token = candidateIds ? candidateIds[scanIndex] : scanIndex;
        if (!vocab?.hasToken?.(token)) continue;
        if (rawLogits[token] > deepMaxRaw) deepMaxRaw = rawLogits[token];
      }
      if (!Number.isFinite(deepMaxRaw)) deepMaxRaw = 0.0;
    }

    const latentUpdates = liveLatentState?.updates || 0;
    const frozenLogitFloor =
      this.reasoningConfig.autoregressiveFrozenLogitFloor ?? 0.30;
    const frozenLogitDecay =
      this.reasoningConfig.autoregressiveFrozenLogitDecay ?? 0.18;
    const frozenLogitWeight =
      deepMode && latentUpdates > 0 && !rawLogitsAreLivePrefix
        ? Math.max(
            frozenLogitFloor,
            Math.exp(-frozenLogitDecay * latentUpdates)
          )
        : 1.0;
    ...
    for (let scanIndex = 0; scanIndex < scanCount; scanIndex++) {
      ...
      // V27: positive normalized neural evidence. The best neural candidate
      // gets 1.0, and alternatives decay smoothly by relative logit distance.
      // Unlike V26, the neural model can now actively WIN a token decision
      // instead of merely assigning zero to the winner and negatives to others.
      const neuralDelta = deepMode
        ? Math.max(-24.0, Math.min(0.0, rawLogits[i] - deepMaxRaw))
        : 0.0;
      const neuralTemperature = Math.max(0.25,
        this.reasoningConfig.rslNeuralRelativeTemperature || 2.0);
      const neuralEvidence = deepMode
        ? Math.exp(neuralDelta / neuralTemperature)
        : 0.0;
      const neuralPositiveWeight = deepMode
        ? (rawLogitsAreLivePrefix
            ? (this.reasoningConfig.rslNeuralPositiveWeight || 5.60)
            : (this.reasoningConfig.rslPromptNeuralPositiveWeight || 3.60))
        : 1.0;
      const neuralContribution = deepMode
        ? neuralEvidence * neuralPositiveWeight * frozenLogitWeight
        : rawLogits[i];

      let score = neuralContribution;
      let knownContribution = neuralContribution;

      const failedTrajectoryContribution = -this.getFailedTrajectorySuppression(
        i,
        prevToken,
        contextModel
      );
      score += failedTrajectoryContribution;
      ...
      const transitionContribution =
        ...
        (deepMode
          ? (this.reasoningConfig.rslLegacyTransitionScale ?? 0.28)
          : 1.0);
      score += transitionContribution;
      knownContribution += transitionContribution;
      ...
      let humanTransitionContribution = 0.0;
      if (humanTransition) {
        const humanScale = deepMode
          ? (this.reasoningConfig.rslHumanTransitionScale ?? 0.42)
          : 1.0;

        // V27: grammatical pairing remains useful, but it no longer carries
        // enough weight to drown out a fresh neural continuation.
        humanTransitionContribution +=
        ...
        score += humanTransitionContribution;
        knownContribution += humanTransitionContribution;
      }
      ...
      const routedSemanticContribution =
        routedSemantic * (deepMode ? 1.55 : 0.70);
      score += routedSemanticContribution;
      knownContribution += routedSemanticContribution;

      let latentContribution = 0.0;
      if (deepMode && liveLatentState) {
        latentContribution = this.getAutoregressiveLatentBonus(
          i,
          liveLatentState,
          contextModel,
          curType
        );
        score += latentContribution;
        knownContribution += latentContribution;
      }
      ...
      // V28 hard lexical-root-family anti-repeat.
      // This now covers both inflection AND derivation:
      // accelerate/accelerating/acceleration/accelerator and
      // explain/explanation/explainable/explainer.
      // One root-family member per clause is enough.
      if (
        sameStemCount > 0 &&
        this.isContentType(curType)
      ) {
      ...
      if (deepMode) {
        ...
        let promptTransitionContribution = 0.0;
        if (prevToken !== null && contextModel.promptTransitions.get(prevToken)?.has(i) && !generatedSet.has(i)) {
          promptTransitionContribution =
            4.6 * (this.reasoningConfig.rslPromptTransitionScale ?? 0.18);
          score += promptTransitionContribution;
          knownContribution += promptTransitionContribution;
        }
        ...
        const roleContribution =
          ...
          (this.reasoningConfig.rslLegacyRoleScale ?? 0.24);
        score += roleContribution;
        knownContribution += roleContribution;

        const lookaheadContribution =
          this.nextRoleLookahead(
            curType,
            sentenceState,
            contextModel
          ) *
          (this.reasoningConfig.rslLegacyLookaheadScale ?? 0.24);
        score += lookaheadContribution;
        knownContribution += lookaheadContribution;

        const answerShapeContribution =
        ...
        score += answerShapeContribution;
        knownContribution += answerShapeContribution;

        const questionRelationContribution =
          this.questionRelationBonus(
            i,
            sentenceState,
            contextModel
          );
        score += questionRelationContribution;
        knownContribution += questionRelationContribution;

        const answerContractContribution =
        ...
        score += answerContractContribution;
        knownContribution += answerContractContribution;

        const answerSlotContribution =
        ...
        score += answerSlotContribution;
        knownContribution += answerSlotContribution;

        const roleSensitiveContribution =
        ...
        score += roleSensitiveContribution;
        knownContribution += roleSensitiveContribution;

        const dynamicPatternContribution = this.dynamicWordPatternScore(
          i,
          generated,
          contextModel,
          sentenceState,
        ...
        score += dynamicPatternContribution;
        knownContribution += dynamicPatternContribution;

        if (decoderDiagnosticsEnabled) {
          // Stash these locals for the final per-token audit below. Function
          // scope is block-local, so place the compact structural subtotal on
          // a token-local scratch object attached to the sink temporarily.
          scoreBreakdownSink.set(i, {
            token: i,
            word,
            type: curType,
            step,
            ...
            transition: transitionContribution,
            humanTransition: humanTransitionContribution,
            routedSemantic: routedSemanticContribution,
            latent: latentContribution,
            promptTransition: promptTransitionContribution,
            role: roleContribution,
            lookahead: lookaheadContribution,
            answerShape: answerShapeContribution,
            questionRelation: questionRelationContribution,
            answerContract: answerContractContribution,
            answerSlot: answerSlotContribution,
            roleSensitive: roleSensitiveContribution,
            dynamicPattern: dynamicPatternContribution,
            livePrefix: rawLogitsAreLivePrefix
          });
        }
      ...
      if (decoderDiagnosticsEnabled) {
        const existing = scoreBreakdownSink.get(i) || {
          token: i,
          word,
          type: curType,
          step,
          ...
          routedSemantic: routedSemanticContribution,
          latent: latentContribution,
          promptTransition: 0.0,
          role: 0.0,
          lookahead: 0.0,
          answerShape: 0.0,
          questionRelation: 0.0,
          answerContract: 0.0,
          answerSlot: 0.0,
          roleSensitive: 0.0,
          dynamicPattern: 0.0,
        ...
        const structuralKnown =
          (existing.transition || 0) +
          (existing.humanTransition || 0) +
          (existing.promptTransition || 0) +
          (existing.role || 0) +
          (existing.lookahead || 0);

        const plannerKnown =
          (existing.answerShape || 0) +
          (existing.questionRelation || 0) +
          (existing.answerContract || 0) +
          (existing.answerSlot || 0) +
          (existing.roleSensitive || 0) +
          (existing.dynamicPattern || 0);

        existing.legacyStructural = structuralKnown;
        existing.planner = plannerKnown;
        existing.other = score -
          (existing.neural || 0) -
          structuralKnown -
          (existing.routedSemantic || 0) -
          (existing.latent || 0) -
          plannerKnown;
        existing.total = score;
        scoreBreakdownSink.set(i, existing);
      }
  ...
  // ========================================================
  // RSL v26: TRUE AUTOREGRESSIVE NEURAL SELF-CONDITIONING + CONSTRAINED SEARCH
  // ========================================================
  //
  // Deep starts with the prompt forward, then every active non-empty beam gets
  // a fresh neural forward on prompt + that beam prefix. The cheap recurrent
  // latent state remains as an auxiliary semantic prior, but neural continuation
  // is no longer frozen or approximated. Live neural top-k candidates expand the
  // compact base pool before RSL grammar, semantic routing and verification.
  //
  ...
  async executeRSLSearch(
    ...
    // High/RSL starts with one prompt forward, then V26 refreshes the real neural
    // state for every non-empty beam prefix. Recovery can still reuse the prompt
    // logits, but continuation itself is now truly autoregressive.
    const rawLogits =
    ...
    let neuralForwardCount =
      reusingNeuralEvidence
        ? 0
        : 1;

    // Prefix cache is search-local. Different beams get different neural states,
    // while identical prefixes never pay for the same forward twice.
    const prefixNeuralCache = new Map();
    ...
    let beams = [
      {
        ...
        latentState: this.createAutoregressiveLatentState(contextModel),
        pathScore: 0.0,
        sentenceCount: 0,
        finished: false,
        priority: 0.0,
    ...
    let peakBeams = 1;
    let peakLiveCandidatePoolSize =
      contextModel?.deepCandidatePool?.length || 0;
    ...
    for (
      ...
      for (const beam of beams) {
        ...
        // V27 keeps the true RL-like feedback loop. The first token uses
        // the prompt forward. Every later token sees prompt + this exact beam's
        // prefix through the real network before RSL applies its constraints.
        let beamRawLogits = rawLogits;
        let rawLogitsAreLivePrefix = false;

        if (
          this.reasoningConfig.rslTrueNeuralSelfConditioningEnabled &&
          beam.tokens.length > 0
        ) {
          const prefixKey = beam.tokens.join(",");
          if (prefixNeuralCache.has(prefixKey)) {
            beamRawLogits = prefixNeuralCache.get(prefixKey);
          } else {
            beamRawLogits = await this.network.forwardSequence(
              cleanPromptTokens.concat(beam.tokens)
            );
            prefixNeuralCache.set(prefixKey, beamRawLogits);
            neuralForwardCount++;
          }
          rawLogitsAreLivePrefix = true;
        }

        const liveCandidatePool =
          this.buildAutoregressiveCandidatePool(
            contextModel.deepCandidatePool,
            beam.tokens,
            contextModel,
            beam.latentState,
        ...
        peakLiveCandidatePoolSize = Math.max(
          peakLiveCandidatePoolSize,
          liveCandidatePool?.length || 0
        );
        ...
        const decoderTopAlternatives = scoreBreakdown
          ? Array.from(scoreBreakdown.values())
        ...
        for (const choice of choices) {
          ...
          const latentState =
            this.advanceAutoregressiveLatentState(
              beam.latentState,
              token,
              contextModel
            );
          ...
          const trajectory =
            beam.trajectory.concat({
              ...
              scoreBreakdown: scoreBreakdown?.get(token)
                ? { ...scoreBreakdown.get(token) }
                : null,
              topAlternatives: decoderTopAlternatives.map(item => ({ ...item })),
          ...
          const driftStop =
            tokens.length >= minimumContinuationLength &&
            Number.isFinite(bestPrefixQuality) &&
      ...
      if (
        ...
        winner = {
          ...
          rslMeta: {
            ...
            trueNeuralSelfConditioningEnabled:
              Boolean(this.reasoningConfig.rslTrueNeuralSelfConditioningEnabled),
            autoregressiveLatentEnabled:
              Boolean(this.reasoningConfig.autoregressiveLatentEnabled),
            autoregressiveLatentUpdates:
              beam.latentState?.updates || 0,
            autoregressiveLatentStability:
              Number.isFinite(beam.latentState?.stability)
                ? beam.latentState.stability
                : null,
            peakLiveCandidatePoolSize,
            rolledBackTokens:
              beam.rolledBackTokens || 0,
    ...
    if (!winner) {
      return {
        ...
        rslMeta: {
          ...
          autoregressiveLatentEnabled:
            Boolean(this.reasoningConfig.autoregressiveLatentEnabled),
          autoregressiveLatentUpdates: 0,
          autoregressiveLatentStability: null,
          peakLiveCandidatePoolSize
        }
      };
    }
  ...
  async executeRollout(
    ...
    // One real neural prompt forward plus cheap latent self-conditioning. The
    // main Deep generator uses executeRSLSearch(); this legacy path preserves
    // the same one-forward latency principle when invoked directly.
    if (deepMode) {
      ...
      let liveLatentState =
        this.createAutoregressiveLatentState(
          contextModel
        );
  ...
  async generate(rawPromptTokens, onProgress = null) {
    ...
    // ================================================================
    // RSL v28 DEEP: TRUE PREFIX NEURAL REFRESH + CONTINUATION-AWARE RSL SEARCH
    // ================================================================
    ...
    let neuralForwardsUsed =
      Math.max(1, winner?.rslMeta?.neuralForwards || 1);
    ...
    return {
      ...
      confidenceDiagnostics: {
        ...
        mode: "deep",
        rslVersion: 28,
        rslPersistenceKey: this.persistenceKey,
        rslLegacyImportEnabled: Boolean(this.reasoningConfig.rslImportLegacyPersistence),
        minimumContinuationLength:
          this.getRSLMinimumContinuationLength(
            rslTargetLength,
            contextModel
          ),
        lexicalRootFamilyEnabled:
          Boolean(this.reasoningConfig.lexicalRootFamilyEnabled),
        decoderWeights: {
          neuralLive: this.reasoningConfig.rslNeuralPositiveWeight,
          neuralPrompt: this.reasoningConfig.rslPromptNeuralPositiveWeight,
          transitionScale: this.reasoningConfig.rslLegacyTransitionScale,
          roleScale: this.reasoningConfig.rslLegacyRoleScale,
          lookaheadScale: this.reasoningConfig.rslLegacyLookaheadScale,
          humanTransitionScale: this.reasoningConfig.rslHumanTransitionScale,
          promptTransitionScale: this.reasoningConfig.rslPromptTransitionScale
        },
        decoderTrace: (winner.trajectory || []).map((stepInfo, index) => ({
          index,
          token: stepInfo.token,
          word: this.network.vocab?.idToToken?.[stepInfo.token] || "",
          prob: stepInfo.prob,
          margin: stepInfo.margin,
          selected: stepInfo.scoreBreakdown || null,
          topAlternatives: stepInfo.topAlternatives || []
        })),
        passesUsed: 1,
        preferredPasses: 1,
        refinementRounds,
        ...
        neuralForwards:
          neuralForwardsUsed,
        autoregressiveLatentEnabled:
          winner.rslMeta?.autoregressiveLatentEnabled ??
          Boolean(this.reasoningConfig.autoregressiveLatentEnabled),
        autoregressiveLatentUpdates:
          winner.rslMeta?.autoregressiveLatentUpdates || 0,
        autoregressiveLatentStability:
          winner.rslMeta?.autoregressiveLatentStability ?? null,
        peakLiveCandidatePoolSize:
          winner.rslMeta?.peakLiveCandidatePoolSize ||
          winner.rslMeta?.candidatePoolSize ||
          0,
        rolledBackTokens:
