...
class VilotNI15Network {
  constructor(backend, vocabSize) {
    ...
    // Only rows explicitly associated with the rewarded response are touched
    // during head-policy feedback. This avoids global head drift and makes
    // feedback substantially cheaper.
    this.rewardHeadTokens = new Set();

    // Incremented whenever embedding values are replaced or adapted.
    // RLEngine uses this to invalidate its normalized semantic cache.
    this.embeddingRevision = 0;
    ...
    // Expose an awaitable readiness gate so consumers cannot snapshot the
    // temporary startup embeddings before persisted synaptic memory is loaded.
    this.ready = this.initSynapticDB();
  }
  ...
  async adaptToPromptTransitions(promptTokens, passes = 2, lr = 0.035) {
    ...
    // The semantic geometry changed. Consumers that cache normalized
    // embeddings must rebuild that cache before the next inference.
    this.embeddingRevision++;
  ...
  async forwardSequence(tokenIds, options = null) {
    const run =
      this._forwardTail.then(
        () =>
          this._forwardSequenceImpl(
            tokenIds,
            options
          )
      );
  ...
  async _forwardSequenceImpl(tokenIds, options = null) {
    this.backend?.assertUsable?.(
      "forwardSequence"
    );
    ...
    try {
      await targetReadback.mapAsync(
        GPUMapMode.READ
      );

      latent =
        new Float32Array(
          targetReadback.getMappedRange()
        ).slice();
    } catch (error) {
      this.backend
        ?.markRuntimeFailure?.(
      ...
      const wrapped =
        new Error(
          "WebGPU readback became invalid during inference."
        );
  ...
  async captureRewardTrace(tokenIds, rewardedTokens = null) {
    ...
    // Clear only rows touched by the previous feedback event.
    for (const tokenId of this.rewardHeadTokens) {
      const off = tokenId * this.poolDim;
      this.headGradients.fill(0, off, off + this.poolDim);
    }
    ...
    const scale = 1.0 / Math.sqrt(Math.max(1, unique.length));

    // Associate the final latent state with the actual generated response
    // tokens. Positive feedback strengthens those rows; negative feedback
    // weakens them. No prompt-specific answer mapping is stored.
  ...
  applyPolicyReward(reward, lr = 0.008, updateDeepLayers = true) {
    ...
    // Reward only head rows that belong to the exact response being rated.
    // This avoids a ~1.2M-element global loop and prevents unrelated outputs
    // from drifting simply because feedback was clicked.
    for (const tokenId of this.rewardHeadTokens) {
      const off = tokenId * this.poolDim;
    ...
    if (!updateDeepLayers || !this.traceFresh) {
      this.rewardHeadTokens.clear();
      return;
    }
