/**
 * VilotNI 2 - MorphologyState.js
 *
 * Contextual grammatical feature resolver. This subsystem does not contain
 * facts or answers. It tracks tense, aspect, modality, mood, voice, agreement,
 * negation, and surface verb realization from the language actually observed.
 */
(() => {
  'use strict';
  ...
  const PAST_SUPPORT = new Set(['did','was','were','had']);
  const FUTURE_SUPPORT = new Set(['will','shall']);
  const CONDITIONAL_SUPPORT = new Set(['would','could','might','should']);

  const clamp = (x, lo = 0, hi = 1) => Math.max(lo, Math.min(hi, Number.isFinite(x) ? x : 0));

  class VilotMorphologyState {
    constructor(processor, options = {}) {
      this.processor = processor;
      this.options = {
        maxFormsPerRoot: Math.max(4, options.languageMaxFormsPerRoot | 0 || 16)
      };
      this.observations = 0;
      this.featureCounts = new Map();
      this.rootForms = new Map();
      this.last = null;
    }

    _inc(map, key, gain = 1) {
      if (!key) return;
      map.set(key, (map.get(key) || 0) + Math.max(0.01, Number(gain) || 1));
    }

    _resolved(tokens) {
      return (tokens || []).filter(t => t && t.pos !== 'Punct');
    }

    analyze(tokens, context = {}) {
      const rows = this._resolved(tokens);
      const lowers = rows.map(t => String(t.lower || '').toLowerCase());
      const modals = lowers.filter(w => MODALS.has(w));
      const auxiliaries = lowers.filter(w => BE.has(w) || HAVE.has(w) || DO.has(w));
      const verbs = rows.filter(t => t.pos === 'Verb');
      const verbLowers = verbs.map(t => t.lower);

      let tense = 'present';
      if (lowers.some(w => FUTURE_SUPPORT.has(w))) tense = 'future';
      else if (lowers.some(w => PAST_SUPPORT.has(w)) || verbLowers.some(w => /ed$/.test(w))) tense = 'past';

      let aspect = 'simple';
      const hasHave = lowers.some(w => HAVE.has(w));
      const hasBeen = lowers.includes('been');
      const hasProgressive = verbLowers.some(w => /ing$/.test(w));
      if (hasHave && hasBeen && hasProgressive) aspect = 'perfect-progressive';
      else if (hasHave) aspect = 'perfect';
      else if (hasProgressive && lowers.some(w => BE.has(w))) aspect = 'progressive';

      let voice = 'active';
      const beIndex = lowers.findIndex(w => BE.has(w));
      if (beIndex >= 0) {
        const after = rows.slice(beIndex + 1);
        if (after.some(t => t.pos === 'Verb' && /(?:ed|en)$/.test(t.lower))) voice = 'passive';
      }

      let mood = context.isQuestion ? 'interrogative' : 'indicative';
      if (!context.isQuestion && rows[0]?.pos === 'Verb') mood = 'imperative';
      if (lowers.some(w => CONDITIONAL_SUPPORT.has(w))) mood = 'conditional';

      const negation = lowers.some(w => w === 'not' || w === 'never' || /n't$/.test(w));
      const modality = modals[0] || null;

      let subjectPerson = 3;
      let subjectNumber = 'singular';
      const subjectWords = context.subjectTokens || [];
      const subjectLower = subjectWords.map(t => String(t.lower || '').toLowerCase());
      if (subjectLower.some(w => w === 'i' || w === 'we')) subjectPerson = 1;
      else if (subjectLower.some(w => w === 'you')) subjectPerson = 2;
      const nominalHeads = subjectWords.filter(t => ['Noun','ProperNoun','Pronoun','Num'].includes(t.pos));
      const head = nominalHeads[nominalHeads.length - 1];
      if (subjectLower.some(w => w === 'we' || w === 'they' || w === 'you') || subjectLower.includes('and') || (head && /s$/.test(head.lower) && !/(ss|us)$/.test(head.lower))) {
      ...
      const result = {
        ...
        subjectNumber,
        auxiliaries,
        verbForms: verbs.map(t => ({
          surface: t.surface,
          lower: t.lower,
          root: t.entry?.root || this.processor?.deriveRoot?.(t.lower) || t.lower,
          pos: t.pos
        }))
      };
      this.last = result;
    ...
    _surfaceEntry(entry, surface) {
      const lower = String(surface || entry?.lower || '').toLowerCase();
      return {
        ...(entry || {}),
        surface: String(surface || entry?.surface || ''),
        lower,
        pos: 'Verb',
        possiblePOS: Array.from(new Set([...(entry?.possiblePOS || []), 'Verb'])),
        grammarInferred: true
      };
    }

    _inflectBase(base, mode) {
      ...
      if (mode === 'progressive') {
        ...
        if (/e$/.test(w) && !/(ee|ye)$/.test(w)) return `${w.slice(0,-1)}ing`;
        return `${w}ing`;
      }
      return w;
    }

    _findOrInfer(entry, mode, baseOverride = '') {
      const found = this.processor?.findVerbForm?.(entry, mode) || null;
      const base = String(baseOverride || entry?.lemma || entry?.root || entry?.lower || '').toLowerCase();
      if (found && found.lower && found.lower !== entry.lower) return found;
      if (mode === 'base') return this._surfaceEntry(entry, base || entry.lower);
      const inferred = this._inflectBase(base || entry.lower, mode);
      return inferred ? this._surfaceEntry(entry, inferred) : (found || entry);
    }

    realizeVerb(tokenOrEntry, features = {}, supportWord = '') {
      const entry = tokenOrEntry?.entry || tokenOrEntry;
      if (!entry) return null;
      const support = String(supportWord || '').toLowerCase();
      const modal = features.modality;
      if (modal || MODALS.has(support)) return this._findOrInfer(entry, 'base', entry.lower);
      if (support === 'did') return this._findOrInfer(entry, 'past', entry.lower);
      if (features.tense === 'past') return this._findOrInfer(entry, 'past');
      if (features.aspect === 'progressive') return this._findOrInfer(entry, 'progressive');
      if (features.tense === 'present' && features.subjectPerson === 3 && features.subjectNumber === 'singular') {
        return this._findOrInfer(entry, 'present3sg', DO.has(support) ? entry.lower : '');
      }
      return this._findOrInfer(entry, 'base');
    }

    observe(tokens, features, gain = 1) {
      const g = Math.max(0.05, Number(gain) || 1);
      if (!features) return;
      this._inc(this.featureCounts, `tense:${features.tense}`, g);
      this._inc(this.featureCounts, `aspect:${features.aspect}`, g);
      this._inc(this.featureCounts, `voice:${features.voice}`, g);
      this._inc(this.featureCounts, `mood:${features.mood}`, g);
      if (features.modality) this._inc(this.featureCounts, `modal:${features.modality}`, g);
      this._inc(this.featureCounts, `negation:${features.negation ? 'yes' : 'no'}`, g);
      for (const row of features.verbForms || []) {
        if (!row.root || !row.lower) continue;
        let map = this.rootForms.get(row.root);
        if (!map) this.rootForms.set(row.root, map = new Map());
        map.set(row.lower, (map.get(row.lower) || 0) + g);
        if (map.size > this.options.maxFormsPerRoot) {
          const weakest = Array.from(map.entries()).sort((a,b) => a[1]-b[1])[0];
    ...
    support(features) {
      if (!features || !this.observations) return 0.5;
      const keys = [
        `tense:${features.tense}`,
        `aspect:${features.aspect}`,
        `voice:${features.voice}`,
        `mood:${features.mood}`,
        `negation:${features.negation ? 'yes' : 'no'}`
      ];
      if (features.modality) keys.push(`modal:${features.modality}`);
      let sum = 0;
      let max = 0;
      for (const value of this.featureCounts.values()) max = Math.max(max, value);
      max = Math.max(1, max);
      for (const key of keys) sum += clamp((this.featureCounts.get(key) || 0) / max);
      return clamp(0.5 + ((sum / keys.length) - 0.5) * clamp(Math.log1p(this.observations) / 6));
    }

    exportState() {
      return {
        observations: this.observations,
        featureCounts: Array.from(this.featureCounts.entries()),
        rootForms: Array.from(this.rootForms.entries()).map(([root, map]) => [root, Array.from(map.entries())])
      };
    }
    ...
    importState(state) {
      ...
      this.featureCounts = new Map(Array.isArray(state.featureCounts) ? state.featureCounts : []);
      this.rootForms = new Map((Array.isArray(state.rootForms) ? state.rootForms : []).map(([root, rows]) => [root, new Map(rows || [])]));
      return true;
    }
    ...
    status() {
      return {
        role: 'contextual-tense-aspect-mood-voice-agreement-state',
        observations: this.observations,
        featurePatterns: this.featureCounts.size,
        learnedVerbRoots: this.rootForms.size,
        last: this.last
      };
    }
  }

  globalThis.VilotMorphologyState = VilotMorphologyState;
})();

/**
 * MorphologyState production research extension.
 *
...
(() => {
  ...
  class VilotMorphologyStateProductionLab {
    constructor(options = {}) {
  ...
  globalThis.VilotMorphologyStateProductionLab = VilotMorphologyStateProductionLab;
})();
