/**
 * VilotNI 2 - Production state arena.
 *
 * The six-million-neuron recurrent address space remains fully allocated and
 * persistent. The arena also tracks which neuron slots have ever become active.
 * Zero is an exact fixed point of the recurrent/homeostasis equations, so slots
 * that have never received drive do not need redundant compute until activated.
 */
(() => {
  'use strict';
  ...
  class VilotArena {
    constructor(options = {}) {
      this.stateDim = Math.max(32, Number(options.stateDim) || 6000000);
      this.maxTokens = Math.max(256, options.maxTokens | 0 || 4096);
      this.maxCandidates = Math.max(16, options.maxCandidates | 0 || 128);
      this.topK = Math.max(4, Math.min(this.maxCandidates, options.topK | 0 || 32));

      // Persistent six-million-neuron ping/pong state.
      this.stateA = new Float32Array(this.stateDim);
      this.stateB = new Float32Array(this.stateDim);
      this.readState = this.stateA;
      this.writeState = this.stateB;

      // Checkpoints are a real engine feature, not a diagnostic convenience.
      this.committedState = new Float32Array(this.stateDim);

      // Exact active-slot tracking. The mask is only one byte per neuron and the
      // slot list grows with actual experience instead of reserving 24 MB up front.
      this.activeMask = new Uint8Array(this.stateDim);
      const initialActiveCapacity = Math.min(
        this.stateDim,
        Math.max(1024, options.initialActiveCapacity | 0 || 65536)
      );
      this.activeSlots = new Uint32Array(initialActiveCapacity);
      this.activeCount = 0;
      this.activeGrowths = 0;

      // Dense external input is unusual in the normal language path. Allocate it
      // only if the public foregroundUpdate API is used.
      this.input = null;

      // Diagnostics are not allowed to consume 48 MB during every normal boot.
      this.benchmarkA = null;
      this.benchmarkB = null;

      this.tokenIds = new Uint32Array(this.maxTokens);
      this.candidateIds = new Uint32Array(this.maxCandidates);
      this.candidateScores = new Float32Array(this.maxCandidates);
      this.topKIds = new Uint32Array(this.topK);
      this.topKScores = new Float32Array(this.topK);
    ...
    _ensureActiveCapacity(required) {
      required = Math.max(0, Math.min(this.stateDim, required | 0));
      if (required <= this.activeSlots.length) return this.activeSlots;
      let next = Math.max(1024, this.activeSlots.length || 1024);
      while (next < required) next = Math.min(this.stateDim, Math.max(required, next * 2));
      const grown = new Uint32Array(next);
      grown.set(this.activeSlots.subarray(0, this.activeCount));
      this.activeSlots = grown;
      this.activeGrowths++;
      return grown;
    }
    ...
    markActive(slot) {
      slot = Number(slot) >>> 0;
      if (slot >= this.stateDim) return false;
      if (this.activeMask[slot]) return false;
      this._ensureActiveCapacity(this.activeCount + 1);
      this.activeMask[slot] = 1;
      this.activeSlots[this.activeCount++] = slot;
      return true;
    }

    markActiveMany(ids, count = ids?.length || 0) {
      if (!ids) return 0;
      const n = Math.max(0, Math.min(ids.length || 0, count | 0));
      let added = 0;
      for (let i = 0; i < n; i++) {
        if (this.markActive(ids[i])) added++;
      }
      return added;
    }

    markAllActive() {
      this._ensureActiveCapacity(this.stateDim);
      for (let i = 0; i < this.stateDim; i++) {
        if (!this.activeMask[i]) this.activeMask[i] = 1;
        this.activeSlots[i] = i;
      }
      this.activeCount = this.stateDim;
    ...
    ensureDenseInput() {
      if (!this.input) this.input = new Float32Array(this.stateDim);
      return this.input;
    }

    clearInput() {
      if (this.input) this.input.fill(0);
    }

    clearWriteState() {
      this.writeState.fill(0);
    }

    ensureBenchmarkBuffers() {
      if (!this.benchmarkA) this.benchmarkA = new Float32Array(this.stateDim);
      if (!this.benchmarkB) this.benchmarkB = new Float32Array(this.stateDim);
      return { a: this.benchmarkA, b: this.benchmarkB };
    }

    releaseBenchmarkBuffers() {
      this.benchmarkA = null;
      this.benchmarkB = null;
    }

    checkpoint() {
      this.committedState.set(this.readState);
      return this.revision;
    }

    restoreCheckpoint() {
      this.readState.set(this.committedState);
      this.writeState.set(this.committedState);
      // Active membership is monotonic between resets. Keeping slots activated
      // after restore is exact (restored zero slots merely cost a future update).
      this.revision++;
    }
    ...
    resetState() {
      this.stateA.fill(0);
      this.stateB.fill(0);
      this.committedState.fill(0);
      this.activeMask.fill(0);
      this.activeCount = 0;
      this.input?.fill(0);
      this.benchmarkA?.fill(0);
      this.benchmarkB?.fill(0);
      this.tokenIds.fill(0);
      this.candidateIds.fill(0);
      this.candidateScores.fill(0);
      this.topKIds.fill(0);
      this.topKScores.fill(0);
      this.tokenCount = 0;
      this.candidateCount = 0;
      this.revision++;
    }

    setInput(source, scale = 1) {
      const dst = this.ensureDenseInput();
      const n = Math.min(dst.length, source?.length || 0);
      dst.fill(0);
      for (let i = 0; i < n; i++) {
        const value = Number.isFinite(source[i]) ? source[i] * scale : 0;
        dst[i] = value;
        if (value !== 0) this.markActive(i);
      }
      return n;
    }
    ...
    stateEnergy() {
      ...
      let sum = 0;
      let peak = 0;
      // Inactive slots are exactly zero, so scanning only active slots returns
      // the same energy while avoiding a six-million-element diagnostic sweep.
      for (let k = 0; k < this.activeCount; k++) {
        const a = Math.abs(state[this.activeSlots[k]]);
        sum += a;
        if (a > peak) peak = a;
      }
      return { meanAbs: sum / Math.max(1, state.length), peak };
    }

    stats() {
      const residentBytes =
        this.stateA.byteLength +
        this.stateB.byteLength +
        this.committedState.byteLength +
        this.activeMask.byteLength +
        this.activeSlots.byteLength +
        (this.input?.byteLength || 0) +
        this.tokenIds.byteLength +
        this.candidateIds.byteLength +
        this.candidateScores.byteLength +
        this.topKIds.byteLength +
        this.topKScores.byteLength;
      const diagnosticBytes = (this.benchmarkA?.byteLength || 0) + (this.benchmarkB?.byteLength || 0);

      return {
        stateDim: this.stateDim,
        neuronCount: this.stateDim,
        activeNeurons: this.activeCount,
        activeRatio: this.activeCount / Math.max(1, this.stateDim),
        activeCapacity: this.activeSlots.length,
        activeGrowths: this.activeGrowths,
        maxTokens: this.maxTokens,
        maxCandidates: this.maxCandidates,
        topK: this.topK,
        residentBytes,
        diagnosticBytes,
        bytes: residentBytes + diagnosticBytes,
        revision: this.revision,
        swapCount: this.swapCount,
        tokenCount: this.tokenCount,
        candidateCount: this.candidateCount,
        denseInputAllocated: Boolean(this.input),
        benchmarkBuffersAllocated: Boolean(this.benchmarkA || this.benchmarkB)
      };
    }
  }

  globalThis.VilotArena = VilotArena;
})();

/**
 * Arena production research extension.
 *
...
(() => {
  ...
  globalThis.VilotArenaProductionLab = VilotArenaProductionLab;
})();
