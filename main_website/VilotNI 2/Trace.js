/**
 * VilotNI 2 - Preallocated production trace ring.
 * Diagnostics allocate only when a snapshot is explicitly requested.
 */
(() => {
  'use strict';
  ...
  const TraceEvent = Object.freeze({
    ...
    DECODER_GENERATE: 22,
    PROCESSOR_AUDIT: 23,
    CORRELATION_STUDY: 24,
    ERROR: 255
  });

  const TraceLabel = new Map(Object.entries(TraceEvent).map(([k, v]) => [v, k]));

  class VilotTrace {
    constructor(capacity = 512) {
      this.capacity = Math.max(64, capacity | 0);
      this.codes = new Uint16Array(this.capacity);
      this.times = new Float64Array(this.capacity);
      this.durations = new Float32Array(this.capacity);
      this.a = new Float32Array(this.capacity);
      this.b = new Float32Array(this.capacity);
      this.c = new Float32Array(this.capacity);
      this.sequence = new Uint32Array(this.capacity);
      this.cursor = 0;
      this.count = 0;
      this.seq = 0;
    }

    push(code, duration = 0, a = 0, b = 0, c = 0) {
      const i = this.cursor;
      this.codes[i] = code;
      ...
      this.b[i] = b;
      this.c[i] = c;
      this.sequence[i] = ++this.seq;
      this.cursor = (i + 1) % this.capacity;
      this.count = Math.min(this.capacity, this.count + 1);
    }

    snapshot(limit = 64) {
      const n = Math.min(this.count, Math.max(1, limit | 0));
      const out = new Array(n);
      const start = (this.cursor - n + this.capacity) % this.capacity;
      for (let j = 0; j < n; j++) {
        const i = (start + j) % this.capacity;
        out[j] = {
          seq: this.sequence[i],
          event: TraceLabel.get(this.codes[i]) || `EVENT_${this.codes[i]}`,
          time: this.times[i],
          durationMs: this.durations[i],
          a: this.a[i],
          b: this.b[i],
          c: this.c[i]
        };
  ...
  globalThis.VilotTraceEvent = TraceEvent;
  globalThis.VilotTrace = VilotTrace;
})();

/**
 * Trace production research extension.
 *
...
(() => {
  ...
  globalThis.VilotTraceProductionLab = VilotTraceProductionLab;
})();
