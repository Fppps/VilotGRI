/**
 * VilotNI 2 - MatrixEngine.js
 *
 * Production reusable candidate-feature matrix engine. It keeps scoring buffers
 * resident so Decoder.js can evaluate large candidate sets without allocating a
 * fresh object graph for every score pass.
 */
(() => {
  'use strict';

  class VilotMatrixEngine {
    constructor(options = {}) {
      this.maxRows = Math.max(64, options.matrixMaxRows | 0 || 512);
      this.maxCols = Math.max(8, options.matrixMaxCols | 0 || 16);
      this.features = new Float32Array(this.maxRows * this.maxCols);
      this.scores = new Float32Array(this.maxRows);
      this.ids = new Uint32Array(this.maxRows);
      this.topIndices = new Uint32Array(Math.max(32, options.matrixTopK | 0 || 64));
      this.topScores = new Float32Array(this.topIndices.length);
      this.rowCount = 0;
      this.colCount = 0;
      this.passes = 0;
      this.rowsScored = 0;
      ...
      if (rows > this.maxRows || cols > this.maxCols) {
        let nextRows = this.maxRows;
        let nextCols = this.maxCols;
        while (nextRows < rows) nextRows <<= 1;
        while (nextCols < cols) nextCols <<= 1;
        this.maxRows = nextRows;
        this.maxCols = nextCols;
        this.features = new Float32Array(this.maxRows * this.maxCols);
        this.scores = new Float32Array(this.maxRows);
        this.ids = new Uint32Array(this.maxRows);
      }
    ...
    setRow(row, id, values) {
      const base = row * this.maxCols;
      this.ids[row] = id >>> 0;
      const n = Math.min(this.colCount, values.length || 0);
      for (let i = 0; i < n; i++) this.features[base + i] = Number(values[i]) || 0;
      for (let i = n; i < this.colCount; i++) this.features[base + i] = 0;
    }

    score(weights, bias = 0, rows = this.rowCount) {
      const started = performance.now();
      const n = Math.min(rows | 0, this.rowCount);
      const cols = this.colCount;
      for (let r = 0; r < n; r++) {
        const base = r * this.maxCols;
        let sum = bias;
        for (let c = 0; c < cols; c++) sum += this.features[base + c] * (weights[c] || 0);
        this.scores[r] = sum;
      }
      this.passes++;
      this.rowsScored += n;
    ...
    topK(k = 8, rows = this.rowCount) {
      const n = Math.min(rows | 0, this.rowCount);
      k = Math.max(1, Math.min(k | 0, n, this.topIndices.length));
      for (let i = 0; i < k; i++) {
        this.topIndices[i] = 0;
        this.topScores[i] = -Infinity;
      }
      for (let r = 0; r < n; r++) {
        const score = this.scores[r];
        if (score <= this.topScores[k - 1]) continue;
        let pos = k - 1;
        while (pos > 0 && score > this.topScores[pos - 1]) {
          this.topScores[pos] = this.topScores[pos - 1];
          this.topIndices[pos] = this.topIndices[pos - 1];
          pos--;
        }
        this.topScores[pos] = score;
        this.topIndices[pos] = r;
      }
      return { indices: this.topIndices, scores: this.topScores, count: k };
    }

    status() {
      return {
        role: 'resident-candidate-feature-matrix',
        maxRows: this.maxRows,
        maxCols: this.maxCols,
        passes: this.passes,
        rowsScored: this.rowsScored,
        lastMs: this.lastMs,
        allocatedBytes: this.features.byteLength + this.scores.byteLength + this.ids.byteLength + this.topIndices.byteLength + this.topScores.byteLength
      };
    }
  }

  globalThis.VilotMatrixEngine = VilotMatrixEngine;
})();

/**
 * MatrixEngine production research extension.
 *
...
(() => {
  ...
  class VilotMatrixEngineProductionLab {
    constructor(options = {}) {
  ...
  globalThis.VilotMatrixEngineProductionLab = VilotMatrixEngineProductionLab;
})();
