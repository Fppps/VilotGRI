/**
 * VilotNI 2 - CandidateRanker.js
 *
 * Fast-path candidate ranking. This module never raises a candidate's reported
 * confidence. The existing Decoder confidence remains the hard emission gate.
 * Ranking only decides which already-generated candidate is worth checking and
 * emitting first when several candidates clear that gate.
 */
(() => {
  'use strict';
  ...
  class VilotCandidateRanker {
    constructor(processor, options = {}) {
      this.processor = processor;
      this.options = {
        historyMax: Math.max(64, options.fastRankHistory | 0 || 256),
        evidenceWords: Math.max(12, options.fastRankEvidenceWords | 0 || 32)
      };
      this.ranks = 0;
      this.history = [];
      this.last = null;
    ...
    _tokens(text) {
      return (this.processor?.tokenize?.(String(text || ''), 192) || [])
        .filter(t => t.pos !== 'Punct');
    }

    _focusCoverage(candidate, plan, evidence) {
      const words = new Set(this._tokens(candidate?.text).map(t => t.lower));
      if (!words.size) return 0;
      const evidenceRows = (evidence?.rows || []).slice(0, this.options.evidenceWords);
      let weightedHit = 0;
      let weightedTotal = 0;
      for (const row of evidenceRows) {
        const weight = Math.max(0.05, Number(row.focusScore) || 0);
        weightedTotal += weight;
        if (words.has(row.entry?.lower)) weightedHit += weight;
      }
      const subject = plan?.subjectWords || [];
      const subjectHit = subject.length ? subject.filter(w => words.has(w)).length / subject.length : 0.5;
      const evidenceHit = weightedTotal ? weightedHit / weightedTotal : 0.5;
      return clamp(evidenceHit * 0.62 + subjectHit * 0.38);
    }

    _directness(candidate, plan) {
      const tokens = this._tokens(candidate?.text);
      if (!tokens.length) return 0;
      const scaffoldLen = plan?.responseWords?.length || 0;
      const extra = Math.max(0, tokens.length - scaffoldLen);
      const role = plan?.requestedRole || 'content';
      const idealExtra = role === 'identity' ? 3 : role === 'truth' ? 2 : role === 'mechanism' || role === 'cause' ? 5 : 4;
      const distance = Math.abs(extra - idealExtra);
      return clamp(1 - distance / Math.max(idealExtra + 5, 8));
    }

    _structuralPenalty(candidate) {
      const tokens = this._tokens(candidate?.text);
      const lowers = tokens.map(t => t.lower);
      let penalty = 0;
      for (let i = 1; i < lowers.length; i++) {
        if (lowers[i] === lowers[i - 1]) penalty += 0.18;
        if (['is','are','was','were'].includes(lowers[i]) && ['is','are','was','were'].includes(lowers[i - 1])) penalty += 0.22;
      }
      const text = String(candidate?.text || '');
      if (/\b(?:is|are|was|were)\s+(?:a|an|the)?\s*(?:is|are|was|were)\b/i.test(text)) penalty += 0.24;
      return clamp(penalty, 0, 0.70);
    }

    _quality(candidate, plan, evidence) {
      const ch = candidate?.diagnostics?.channels || {};
      const confidence = clamp((Number(candidate?.confidence) || 0) / 100);
      const grammar = clamp(ch.grammarLegality ?? 0);
      const complete = clamp(ch.sentenceCompleteness ?? 0);
      const contract = clamp(ch.answerContract ?? 0);
      const scaffold = clamp(ch.scaffoldFit ?? 0);
      const language = clamp(ch.languageStateSupport ?? 0.5);
      const pos = clamp(ch.dynamicPOSCoherence ?? 0.5);
      const trajectory = clamp(ch.trajectoryCertainty ?? 0.5);
      const lexical = clamp(ch.lexicalAdaptation ?? 0.5);
      const focus = this._focusCoverage(candidate, plan, evidence);
      const directness = this._directness(candidate, plan);
      const penalty = this._structuralPenalty(candidate);
      const quality = clamp(
        confidence * 0.34 +
        grammar * 0.12 +
        complete * 0.10 +
    ...
    rank(candidates, plan, evidence, minConfidence = 80) {
      const rows = (candidates || []).map((candidate, index) => {
        const metrics = this._quality(candidate, plan, evidence);
        return {
          candidate,
          index,
          metrics,
        ...
        if (a.clearsGate !== b.clearsGate) return a.clearsGate ? -1 : 1;
        if (b.quality !== a.quality) return b.quality - a.quality;
        return (Number(b.candidate?.confidence) || 0) - (Number(a.candidate?.confidence) || 0);
      });

      const selected = rows.find(row => row.clearsGate)?.candidate || null;
      const best = rows[0]?.candidate || null;
      const bestObservedConfidence = Math.max(0, ...rows.map(row => Number(row.candidate?.confidence) || 0));
      const summary = {
        ...
        bestQuality: rows[0]?.quality || 0,
        top: rows.slice(0, 8).map(row => ({
          confidence: Number(row.candidate?.confidence) || 0,
          quality: Number(row.quality.toFixed(4)),
          focus: Number(row.metrics.focus.toFixed(4)),
          grammar: Number(row.metrics.grammar.toFixed(4)),
          complete: Number(row.metrics.complete.toFixed(4)),
          contract: Number(row.metrics.contract.toFixed(4)),
          penalty: Number(row.metrics.penalty.toFixed(4)),
          text: String(row.candidate?.text || '').slice(0, 160)
        }))
      };
      this.ranks++;
      this.last = summary;
      ...
      return { rows, selected, best, summary };
    }

    status() {
      return {
        role: 'non-inflating-fast-candidate-ranker',
        ranks: this.ranks,
        last: this.last
      };
    }
  ...
  globalThis.VilotCandidateRanker = VilotCandidateRanker;
})();

/**
 * CandidateRanker production research extension.
 *
...
(() => {
  ...
  class VilotCandidateRankerProductionLab {
    constructor(options = {}) {
    ...
    status() {
      return {
        ...
        role: 'candidate-ranking-production-research-extension',
        calls: this.calls,
  ...
  globalThis.VilotCandidateRankerProductionLab = VilotCandidateRankerProductionLab;
})();
