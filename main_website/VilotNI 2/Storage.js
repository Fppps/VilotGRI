/**
 * VilotNI 2 - Production persistence layer.
 *
 * Persists the production learning bundle: RSL plus WordState, PhraseState,
 * WordMatrix, LanguageState and Correlation curiosity state. Saves are coalesced so a burst of responses does not repeatedly
 * serialize the same learned state.
 */
(() => {
  'use strict';
  ...
  class VilotStorage {
    constructor(options = {}) {
      this.dbName = options.dbName || 'VilotNI2_Engine';
      this.storeName = options.storeName || 'state';
      this.key = options.key || 'rsl-learning-schema-8';
      this.version = 1;
      this.db = null;
      this.ready = false;
      this.lastSaveMs = 0;
      ...
      this.saveCount = 0;
      this.loadCount = 0;
      this.saveDelayMs = Math.max(500, options.saveDelayMs | 0 || 1800);
    }
    ...
    async flush() {
      if (this.saveInFlight) {
        try { await this.saveInFlight; } catch (_) {}
      }
      const producer = this.pendingProducer;
      this.pendingProducer = null;
      if (!producer) return false;
      try {
        const value = typeof producer === 'function' ? producer() : producer;
        this.saveInFlight = this.save(value);
    ...
    status() {
      return {
        ...
        savePending: Boolean(this.pendingTimer || this.pendingProducer || this.saveInFlight),
        persistent: this.ready
      };
    }
  }

  globalThis.VilotStorage = VilotStorage;
})();

/**
 * Storage production research extension.
 *
...
(() => {
  ...
  class VilotStorageProductionLab {
    constructor(options = {}) {
  ...
  globalThis.VilotStorageProductionLab = VilotStorageProductionLab;
})();
