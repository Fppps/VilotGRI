/**
 * VilotNI 2 - PhraseState.js
 *
 * Learned multi-word state. This does not store canned answers. It stores how
 * strongly short observed word/POS sequences have behaved as coherent units.
 */
(() => {
  'use strict';
  ...
  class VilotPhraseState {
    constructor(lexicon, options = {}) {
      this.lexicon = lexicon;
      this.minN = 2;
      this.maxN = Math.max(2, Math.min(5, options.phraseMaxN | 0 || 4));
      this.maxPhrases = Math.max(4096, options.phraseMaxEntries | 0 || 32768);
      this.map = new Map();
      this.updates = 0;
      this.prunes = 0;
    ...
    _key(words) { return words.join(SEP); }
    _touch(words, pos, metadata = {}) {
      const key = this._key(words);
      let row = this.map.get(key);
      if (!row) {
        row = { strength: 0.5, success: 0, failure: 0, observations: 0, pos: (pos || []).join('>'), last: 0 };
      ...
      const gain = Math.max(0.02, Math.min(2.5, Number(metadata.gain) || 1));
      row.observations += gain;
      if (accepted) row.success += gain * Math.max(0.2, quality);
      else row.failure += gain * Math.max(0.2, 1 - quality * 0.5);
      const target = accepted ? 0.55 + quality * 0.45 : 0.5 - (1 - quality) * 0.42;
      const alpha = Math.min(0.30, 0.045 * gain + 0.02);
      row.strength = clamp(row.strength * (1 - alpha) + target * alpha);
      row.last = this.updates + 1;
      this.updates++;
    }

    observeTokens(tokens, metadata = {}) {
      const rows = (tokens || []).filter(t => t && t.word);
      if (rows.length < 2) return { phrases: 0 };
      let count = 0;
      for (let n = this.minN; n <= this.maxN; n++) {
        for (let i = 0; i + n <= rows.length; i++) {
          const slice = rows.slice(i, i + n);
          this._touch(slice.map(x => x.word), slice.map(x => x.pos || 'Other'), metadata);
          count++;
        }
      }
      if (this.map.size > this.maxPhrases) this._prune();
      return { phrases: count };
    }

    observeText(text, processor, rsl, metadata = {}) {
      const raw = (processor?.tokenize?.(String(text || ''), 256) || []).filter(t => t.pos !== 'Punct');
      const state = { previousWord: '', previousPOS: 'START2', lastWord: '', lastPOS: 'START' };
      const rows = [];
      for (const token of raw) {
        const res = rsl?.resolvePOS?.(token.entry, state, { fallbackPOS: token.pos }) || { pos: token.pos };
        rows.push({ word: token.lower, pos: res.pos || token.pos });
        state.previousWord = state.lastWord; state.previousPOS = state.lastPOS;
        state.lastWord = token.lower; state.lastPOS = res.pos || token.pos;
      }
      return this.observeTokens(rows, metadata);
    }

    candidateSupport(prefixWords, candidateWord) {
      const prefix = (prefixWords || []).map(x => String(x || '').toLowerCase()).filter(Boolean);
      const candidate = String(candidateWord || '').toLowerCase();
      if (!candidate) return 0.5;
      let sum = 0, n = 0;
      for (let len = 1; len < this.maxN; len++) {
        if (prefix.length < len) continue;
        const words = prefix.slice(-len).concat(candidate);
        const row = this.map.get(this._key(words));
        if (row) { sum += row.strength; n++; }
      }
      return n ? clamp(sum / n) : 0.5;
    }
    ...
    coherence(text, processor) {
      ...
      let sum = 0, n = 0;
      for (let len = 2; len <= this.maxN; len++) {
        for (let i = 0; i + len <= words.length; i++) {
          const row = this.map.get(this._key(words.slice(i, i + len)));
          if (row) { sum += row.strength; n++; }
        }
      }
    ...
    _prune() {
      const target = Math.floor(this.maxPhrases * 0.88);
      const rows = Array.from(this.map.entries());
      rows.sort((a, b) => {
        const av = Math.abs((a[1].strength || 0.5) - 0.5) * 2 + Math.log1p(a[1].observations || 0) * 0.06;
        const bv = Math.abs((b[1].strength || 0.5) - 0.5) * 2 + Math.log1p(b[1].observations || 0) * 0.06;
        return bv - av;
      });
      this.map = new Map(rows.slice(0, target));
      this.prunes++;
    }

    exportState() {
      return { schemaVersion: 1, maxN: this.maxN, entries: Array.from(this.map.entries()), updates: this.updates, prunes: this.prunes };
    }
    importState(state) {
      if (!state || !Array.isArray(state.entries)) return false;
      this.map = new Map(state.entries.slice(0, this.maxPhrases));
      this.updates = Math.max(0, Number(state.updates) || 0);
      this.prunes = Math.max(0, Number(state.prunes) || 0);
      return true;
    }
    reset() { this.map.clear(); this.updates = 0; this.prunes = 0; this.lastCoherence = 0.5; }
    status() { return { role: 'learned-short-phrase-state', phrases: this.map.size, updates: this.updates, prunes: this.prunes, maxN: this.maxN, lastCoherence: this.lastCoherence, storesCannedAnswers: false }; }
  }

  globalThis.VilotPhraseState = VilotPhraseState;
})();

/**
 * PhraseState production research extension.
 *
...
(() => {
  ...
  class VilotPhraseStateProductionLab {
    constructor(options = {}) {
  ...
  globalThis.VilotPhraseStateProductionLab = VilotPhraseStateProductionLab;
})();
