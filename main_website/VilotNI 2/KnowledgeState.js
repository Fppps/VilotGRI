/**
 * VilotNI 2 - KnowledgeState.js
 *
 * Epistemic boundary tracking for novel/unknown terms. This subsystem separates
 * grammatical recognition from semantic knowledge so VilotNI can confidently
 * report that it does not yet know a concept instead of hallucinating until the
 * confidence gate is satisfied.
 */
(() => {
  'use strict';

  const FUNCTION_POS = new Set(['Det','Aux','Modal','Prep','Conj','Punct']);
  const STOP = new Set(['what','which','who','whom','whose','where','when','why','how','is','are','am','was','were','do','does','did','can','could','will','would','should','may','might','must','shall','have','has','had','a','an','the']);

  class VilotKnowledgeState {
    constructor(processor, lexicon, wordState, phraseState, rsl, options = {}) {
      ...
      this.unknownTerms = new Map();
      this.observations = 0;
      this.last = null;
    }

    _unknownToken(token) {
      if (!token || token.pos === 'Punct' || FUNCTION_POS.has(token.pos) || STOP.has(token.lower)) return false;
      if (token.entry?.guessed) return true;
      return !this.lexicon?.id?.(token.lower);
    }
    ...
    _groups(words) {
      ...
      for (const token of words || []) {
        if (this._unknownToken(token)) current.push(token);
        else flush();
      }
      flush();
      return groups;
    ...
    inspect(analysis, language = {}) {
      const clause = language?.clause || null;
      const roleWords = clause
        ? [...(clause.subjectTokens || []), ...(clause.objectTokens || [])]
        : (analysis?.words || []);
      const groups = this._groups(roleWords);
      const unknown = groups.map(group => ({
        text: group.map(t => t.surface).join(' '),
        lower: group.map(t => t.lower).join(' '),
        pos: group.map(t => t.pos),
        lexicalStatus: 'unseen'
      }));

      for (const item of unknown) {
        const old = this.unknownTerms.get(item.lower) || { occurrences: 0, pos: {}, userDescriptions: 0, firstSeen: Date.now(), lastSeen: 0 };
        old.occurrences++;
        old.lastSeen = Date.now();
        for (const pos of item.pos) old.pos[pos] = (old.pos[pos] || 0) + 1;
        this.unknownTerms.set(item.lower, old);
      }

      const totalContent = Math.max(1, (analysis?.content || []).length);
      const knownContent = (analysis?.content || []).filter(t => !this._unknownToken(t)).length;
      const knownWordRatio = knownContent / totalContent;
      let status = unknown.length ? 'lexically-known-grammar-semantically-unknown' : 'known-or-partially-known';
      let shouldWithholdSemanticAnswer = false;
      if (unknown.length && analysis?.isQuestion) shouldWithholdSemanticAnswer = true;

      const confidence = unknown.length
        ? Math.min(0.995, 0.90 + Math.min(0.08, unknown.length * 0.025) + (1 - knownWordRatio) * 0.02)
        : 0.5;
      ...
      const result = {
        ...
        knownWordRatio,
        shouldWithholdSemanticAnswer,
        unknownStateConfidence: confidence,
        requestedRole: analysis?.requestedSlot || 'content'
      };
      this.observations++;
      this.last = result;
      return result;
    ...
    noteUserDescription(analysis) {
      if (!analysis || analysis.isQuestion) return;
      const words = analysis.words || [];
      const unknown = this._groups(words);
      for (const group of unknown) {
        const key = group.map(t => t.lower).join(' ');
        const state = this.unknownTerms.get(key);
        if (state) state.userDescriptions = (state.userDescriptions || 0) + 1;
      }
    }

    unknownEvidence(analysis, language = {}) {
      const knowledge = this.inspect(analysis, language);
      return {
        status: knowledge.status,
        unknownTerms: knowledge.unknownTerms,
        knownWordRatio: knowledge.knownWordRatio,
        requestedRole: knowledge.requestedRole,
        uncertainty: knowledge.unknownTerms.length
          ? Math.max(0, 1 - knowledge.knownWordRatio)
          : 0,
        source: 'learned-state-observation'
      };
    }

    exportState() { return { observations:this.observations, unknownTerms:Array.from(this.unknownTerms.entries()) }; }
    importState(state) { if (!state) return false; this.observations=Number(state.observations)||0; this.unknownTerms=new Map(state.unknownTerms||[]); return true; }
    reset() { this.observations=0; this.unknownTerms.clear(); this.last=null; }
    status() { return { role:'lexical-vs-semantic-knowledge-boundary', observations:this.observations, trackedUnknownTerms:this.unknownTerms.size, last:this.last }; }
  }

  globalThis.VilotKnowledgeState = VilotKnowledgeState;
})();

/**
 * KnowledgeState production research extension.
 *
...
(() => {
  ...
  class VilotKnowledgeStateProductionLab {
    constructor(options = {}) {
      this.options = { ...options };
      this.calls = 0;
  ...
  globalThis.VilotKnowledgeStateProductionLab = VilotKnowledgeStateProductionLab;
})();
