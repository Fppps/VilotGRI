/**
 * VilotNI 2 - Lexicon.js
 *
 * Production lexical index built directly from Words.json after Processor.js loads it.
 * This file contains no answers and no subject facts. It gives the learned word
 * subsystems stable integer IDs, POS metadata, roots, and fast lookup tables.
 */
(() => {
  'use strict';

  const POS_ORDER = Object.freeze([
    'Other','Noun','ProperNoun','Verb','Adj','Adv','Det','Pronoun','Aux','Modal',
    'Prep','Conj','Num','Punct','Existential','Interjection','Foreign'
  ]);
  ...
  class VilotLexicon {
    constructor(processor, options = {}) {
      ...
      this.idToWord = [''];
      this.idToEntry = [null];
      this.posToId = new Map(POS_ORDER.map((pos, i) => [pos, i]));
      this.idToPOS = POS_ORDER.slice();
      this.rootToIds = new Map();
      this.categoryToIds = new Map();
      this.unknownLookups = 0;
      this.buildMs = 0;
      this.version = 1;
      this._build();
    }
    ...
    _build() {
      ...
      const rows = Array.from(this.processor?.entries?.values?.() || [])
        .filter(entry => entry && entry.active !== false && entry.lower)
        .sort((a, b) => a.lower.localeCompare(b.lower));
      for (const entry of rows) {
        if (this.wordToId.has(entry.lower)) continue;
        const id = this.idToWord.length;
        this.wordToId.set(entry.lower, id);
        this.idToWord.push(entry.lower);
        this.idToEntry.push(entry);
        if (entry.root) {
          let list = this.rootToIds.get(entry.root);
          if (!list) this.rootToIds.set(entry.root, list = []);
          list.push(id);
        }
        if (entry.category) {
          let list = this.categoryToIds.get(entry.category);
          if (!list) this.categoryToIds.set(entry.category, list = []);
          list.push(id);
        }
        for (const pos of entry.possiblePOS || [entry.pos]) this.ensurePOS(pos);
      }
      this.buildMs = performance.now() - started;
    }

    ensurePOS(pos) {
      const key = String(pos || 'Other');
      if (this.posToId.has(key)) return this.posToId.get(key);
      const id = this.idToPOS.length;
      this.posToId.set(key, id);
      this.idToPOS.push(key);
      return id;
    }

    id(word) {
      const key = String(word || '').toLowerCase();
      const id = this.wordToId.get(key) || 0;
      if (!id && key) this.unknownLookups++;
      return id;
    }

    word(id) { return this.idToWord[id | 0] || ''; }
    entry(idOrWord) {
      const id = typeof idOrWord === 'number' ? idOrWord | 0 : this.id(idOrWord);
      return this.idToEntry[id] || null;
    }
    posId(pos) { return this.ensurePOS(pos); }
    posName(id) { return this.idToPOS[id | 0] || 'Other'; }
    get size() { return Math.max(0, this.idToWord.length - 1); }
    get posCount() { return this.idToPOS.length; }

    ids(words, limit = 64) {
      const out = [];
      for (const word of words || []) {
        const id = this.id(word);
        if (id) out.push(id);
    ...
    status() {
      return {
        role: 'production-static-lexical-index',
        words: this.size,
        posTypes: this.posCount,
        roots: this.rootToIds.size,
        categories: this.categoryToIds.size,
        buildMs: this.buildMs,
        unknownLookups: this.unknownLookups,
        containsAnswers: false
      };
    }
  }

  globalThis.VilotLexicon = VilotLexicon;
})();

/**
 * Lexicon production research extension.
 *
...
(() => {
  ...
  class VilotLexiconProductionLab {
    constructor(options = {}) {
  ...
  globalThis.VilotLexiconProductionLab = VilotLexiconProductionLab;
})();
