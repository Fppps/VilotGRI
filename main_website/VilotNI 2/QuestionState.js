/**
 * VilotNI 2 - QuestionState.js
 *
 * General interrogative role analysis. It represents question structure rather
 * than storing question/answer templates. New lexical content can occupy the
 * same learned grammatical roles without a per-question rule.
 */
(() => {
  'use strict';

  const WH = new Set(['what','which','who','whom','whose','where','when','why','how']);
  ...
  class VilotQuestionState {
    ...
    classify(tokens, baseAnalysis = {}) {
      const words = (tokens || []).filter(t => t?.pos !== 'Punct');
      const lowers = words.map(t => String(t.lower || '').toLowerCase());
      const first = lowers[0] || '';
      const isQuestion = Boolean(baseAnalysis.isQuestion || WH.has(first) || SUPPORT.has(first));
      const wh = WH.has(first) ? first : null;
      let compound = null;
      if (wh === 'how' && ['many','much','long','fast','often','likely'].includes(lowers[1])) compound = `how_${lowers[1]}`;
      else if (wh === 'what' && ['kind','type','happens'].includes(lowers[1])) compound = `what_${lowers[1]}`;
      else if (wh === 'which' && lowers[1]) compound = 'which_selection';

      let family = 'statement';
      let requestedRole = baseAnalysis.requestedSlot || 'content';
      if (isQuestion) {
        if (!wh) family = 'yes_no';
        else if (compound) family = compound;
        else if (wh === 'how') family = 'mechanism';
        else if (wh === 'why') family = 'cause';
        else if (wh === 'when') family = 'time';
        else if (wh === 'where') family = 'location';
        else if (wh === 'who' || wh === 'whom' || wh === 'whose') family = 'person';
        else if (wh === 'which') family = 'selection';
        else if (wh === 'what') family = baseAnalysis.intent === 'definition' ? 'definition' : 'content';
        else family = 'question';
      }

      if (compound === 'how_many' || compound === 'how_much') requestedRole = 'quantity';
      else if (compound === 'how_long') requestedRole = 'duration';
      else if (compound === 'how_fast') requestedRole = 'rate';
      else if (compound === 'how_often') requestedRole = 'frequency';
      else if (compound === 'how_likely') requestedRole = 'probability';
      else if (family === 'mechanism') requestedRole = 'mechanism';
      else if (family === 'cause') requestedRole = 'cause';
      else if (family === 'time') requestedRole = 'time';
      else if (family === 'location') requestedRole = 'location';
      else if (family === 'person') requestedRole = 'person';
      else if (family === 'selection') requestedRole = 'selection';
      else if (family === 'yes_no') requestedRole = 'truth';
      else if (family === 'definition') requestedRole = 'identity';

      const supportIndex = isQuestion ? words.findIndex((t, i) => i > (compound ? 1 : wh ? 0 : -1) && (t.pos === 'Aux' || t.pos === 'Modal' || SUPPORT.has(t.lower))) : -1;
      const result = {
        isQuestion,
        wh,
        compound,
        ...
        requestedRole,
        supportIndex,
        supportWord: supportIndex >= 0 ? words[supportIndex].lower : null
      };
      this.last = result;
      return result;
    }
    ...
    observe(state, gain = 1) {
      if (!state) return;
      const key = `${state.family}|${state.supportWord || 'none'}|${state.requestedRole || 'content'}`;
      this.familyCounts.set(key, (this.familyCounts.get(key) || 0) + Math.max(0.05, Number(gain) || 1));
      this.observations++;
    }

    support(state) {
      if (!state || !this.observations) return 0.5;
      const key = `${state.family}|${state.supportWord || 'none'}|${state.requestedRole || 'content'}`;
      const count = this.familyCounts.get(key) || 0;
      return Math.min(1, 0.5 + Math.log1p(count) / 8);
    }

    exportState() { return { observations: this.observations, familyCounts: Array.from(this.familyCounts.entries()) }; }
    importState(state) { if (!state) return false; this.observations = Number(state.observations)||0; this.familyCounts = new Map(state.familyCounts || []); return true; }
    reset() { this.observations = 0; this.familyCounts.clear(); this.last = null; }
    status() { return { role: 'general-interrogative-role-state', observations: this.observations, learnedFamilies: this.familyCounts.size, last: this.last }; }
  }

  globalThis.VilotQuestionState = VilotQuestionState;
})();

/**
 * QuestionState production research extension.
 *
...
(() => {
  ...
  class VilotQuestionStateProductionLab {
    constructor(options = {}) {
      this.options = { ...options };
      this.calls = 0;
  ...
  globalThis.VilotQuestionStateProductionLab = VilotQuestionStateProductionLab;
})();
