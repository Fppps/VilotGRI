/**
 * VilotNI 2 - QueryPlanner.js
 *
 * Fast, non-factual request planning. This module does not contain answers.
 * It converts the already parsed LanguageState analysis into a compact search
 * contract so the foreground path knows what evidence to retrieve first and
 * what grammatical/semantic shape a successful answer must satisfy.
 */
(() => {
  'use strict';
  ...
  const PRONOUNS = new Set(['it','this','that','these','those','he','she','they','them','him','her','there','here','one','ones','something','someone']);
  const QUESTION_OPERATORS = new Set(['what','which','who','whom','whose','where','when','why','how']);
  ...
  const CONTEXT_FILLERS = new Set(['not','so','then','else','again','really','exactly','though','too','either','instead','about','and','but','also','still','just']);

  class VilotQueryPlanner {
    constructor(processor, rsl, languageState = null, options = {}, ruleStore = null) {
      this.processor = processor;
      this.rsl = rsl;
      this.languageState = languageState;
      this.ruleStore = ruleStore;
      this.options = {
        maxSeeds: Math.max(6, options.queryPlannerMaxSeeds | 0 || 18),
        maxSubjectWords: Math.max(2, options.queryPlannerMaxSubjectWords | 0 || 8),
        focusedBatchMin: Math.max(2, options.fastFocusedBatchMin | 0 || 3),
        focusedBatchMax: Math.max(4, options.fastFocusedBatchMax | 0 || 6),
        lexicalFastFloor: clamp(Number(options.queryPlannerLexicalFastFloor) || 0.56),
        planHistory: Math.max(64, options.queryPlannerHistory | 0 || 256)
      };
      this.planCount = 0;
      this.fastEligibleCount = 0;
      this.history = [];
      this.last = null;
    }

    _wordTokens(analysis) {
      return (analysis?.words || []).filter(t => t && t.pos !== 'Punct');
    }

    _contentTokens(analysis) {
      return (analysis?.content || []).filter(Boolean);
    }

    _subjectWords(analysis) {
      const scaffoldSubject = analysis?.responseScaffold?.subject || [];
      if (scaffoldSubject.length) return uniq(scaffoldSubject).slice(0, this.options.maxSubjectWords);
      const clauseSubject = analysis?.clauseState?.subjectTokens || [];
      if (clauseSubject.length) return uniq(clauseSubject.map(t => t.lower)).slice(0, this.options.maxSubjectWords);

      // For short definition/content questions, the final content phrase usually
      // carries the concept being requested. This is structural extraction only.
      const content = this._contentTokens(analysis)
        .map(t => String(t.lower || '').toLowerCase())
        .filter(w => w && !PRONOUNS.has(w) && !QUESTION_OPERATORS.has(w) && !SUPPORT.has(w) && !CONTEXT_FILLERS.has(w));
      if (content.length) return uniq(content).slice(-this.options.maxSubjectWords);
      return [];
    }

    _predicateWords(analysis) {
      const out = [];
      const scaffold = analysis?.responseScaffold;
      if (scaffold?.predicate) out.push(scaffold.predicate);
      const predicate = analysis?.clauseState?.predicateToken?.lower;
      if (predicate) out.push(predicate);
      return uniq(out);
    }

    _objectWords(analysis) {
      const object = analysis?.clauseState?.objectTokens || [];
      return uniq(object.map(t => t.lower));
    }

    _roleBridges(role) {
      // These words describe grammatical answer roles, not domain facts.
      const external = this.ruleStore?.roleBridges?.(role);
      if (Array.isArray(external) && external.length) return external;
      if (role === 'identity') return ['type','kind','system','process','concept','object'];
      if (role === 'mechanism') return ['by','through','using','with','process','method'];
      if (role === 'cause') return ['because','cause','result','from','through'];
      if (role === 'location') return ['in','at','on','inside','within'];
      if (role === 'time') return ['during','before','after','when','while'];
      if (role === 'quantity') return ['number','amount','many','much'];
      if (role === 'rate') return ['rate','speed','per'];
      if (role === 'duration') return ['time','duration','long'];
      if (role === 'frequency') return ['often','frequency','times'];
      if (role === 'probability') return ['likely','chance','probability'];
      if (role === 'truth') return ['is','are','does','do','can','will'];
      return [];
    }

    _contextDependency(analysis, subjectWords) {
      const words = this._wordTokens(analysis);
      const lowers = words.map(t => String(t.lower || '').toLowerCase());
      const joined = lowers.join(' ').replace(/[?!.,;:]+$/g, '').trim();
      const contextData = this.ruleStore?.contextRules?.() || {};
      const selfRules = this.ruleStore?.selfContainedRules?.() || {};
      const pronouns = new Set(Array.isArray(contextData.Pronouns) ? contextData.Pronouns : Array.from(PRONOUNS));
      const operators = new Set(Array.isArray(contextData.Question_Operators) ? contextData.Question_Operators : Array.from(QUESTION_OPERATORS));
      const support = new Set(Array.isArray(contextData.Support_Verbs) ? contextData.Support_Verbs : Array.from(SUPPORT));
      const fillers = new Set(Array.isArray(contextData.Fillers) ? contextData.Fillers : Array.from(CONTEXT_FILLERS));
      const exactRule = this.ruleStore?.exactEllipsis?.(joined) || null;
      const content = this._contentTokens(analysis);
      const pronounContent = content.filter(t => pronouns.has(String(t.lower || '').toLowerCase()));
      const concreteContent = content.filter(t => {
        const lower = String(t.lower || '').toLowerCase();
        return !pronouns.has(lower) && !operators.has(lower) && !support.has(lower) && !fillers.has(lower);
      });
      const shortLimit = Math.max(2, Number(selfRules.short_wh_token_limit) || 5);
      const ellipsisLimit = Math.max(2, Number(selfRules.discourse_ellipsis_token_limit) || 4);
      const veryShort = lowers.length <= 2;
      const operatorOnly = veryShort && lowers.every(w => operators.has(w) || support.has(w) || pronouns.has(w) || fillers.has(w));
      const startsWithOperator = operators.has(lowers[0] || '');
      const hasNamedOrLexicalSubject = subjectWords.some(w => !pronouns.has(w) && !fillers.has(w));
      const noConcreteAnchor = concreteContent.length === 0 && !hasNamedOrLexicalSubject;
      const shortEllipsis = lowers.length <= shortLimit && startsWithOperator && noConcreteAnchor;
      const discourseEllipsis = lowers.length <= ellipsisLimit && lowers.length > 0 && lowers.every(w =>
        operators.has(w) || support.has(w) || pronouns.has(w) || fillers.has(w)
      );
      const deicticOnly = concreteContent.length === 0 && pronounContent.length > 0;
      const requiresContext = Boolean(
        exactRule?.requires_context === true ||
        operatorOnly ||
        shortEllipsis ||
        discourseEllipsis ||
        (deicticOnly && !hasNamedOrLexicalSubject)
      );
      let reason = 'lexically-anchored';
      if (exactRule?.requires_context === true) reason = `exact-ellipsis:${exactRule.relation || 'context'}`;
      else if (operatorOnly) reason = 'operator-only';
      else if (shortEllipsis || discourseEllipsis) reason = 'elliptical-question';
      else if (deicticOnly && !hasNamedOrLexicalSubject) reason = 'deictic-reference';
      return {
        requiresContext,
        selfContained: !requiresContext,
        reason,
        relation: exactRule?.relation || null,
        pronounCount: pronounContent.length,
        concreteContentCount: concreteContent.length,
        tokenCount: lowers.length,
        startsWithOperator,
        noConcreteAnchor
      };
    }
    ...
    _complexity(analysis) {
      const words = this._wordTokens(analysis);
      const contentCount = this._contentTokens(analysis).length;
      const clauseCount = Math.max(1, Number(analysis?.clauseState?.clauseCount) || 1);
      const unknownPenalty = 1 - clamp(analysis?.lexicalCoverage ?? 0);
      const role = analysis?.requestedSlot || 'content';
      const roleCost = ['mechanism','cause','selection'].includes(role) ? 0.18 : ['identity','truth'].includes(role) ? 0.04 : 0.10;
      return clamp(
        0.10 +
        Math.min(0.35, words.length / 48) +
        Math.min(0.20, contentCount / 24) +
        Math.min(0.15, (clauseCount - 1) * 0.08) +
        unknownPenalty * 0.20 + roleCost
      );
    }

    _candidateBudget(complexity, role) {
      const span = this.options.focusedBatchMax - this.options.focusedBatchMin;
      let budget = this.options.focusedBatchMin + Math.round(complexity * span);
      if (role === 'identity' || role === 'truth') budget = Math.max(this.options.focusedBatchMin, budget - 1);
      return Math.max(this.options.focusedBatchMin, Math.min(this.options.focusedBatchMax, budget));
    }

    _expectedPOS(analysis) {
      const pos = (analysis?.responseScaffold?.pos || []).map(x => String(x || '')).filter(Boolean);
      return pos.slice(0, 32);
    }

    _focusSeeds(analysis, subjectWords, predicateWords, objectWords, role) {
      const content = (analysis?.contentWords || []).slice(0, this.options.maxSeeds);
      const roots = (analysis?.roots || []).slice(0, this.options.maxSeeds);
      const bridges = this._roleBridges(role);
      // Subject and explicit content outrank bridge words later in EvidenceSearch.
      return uniq([...subjectWords, ...predicateWords, ...objectWords, ...content, ...roots, ...bridges]).slice(0, this.options.maxSeeds);
    }

    _anchorPOS(subjectWords) {
      for (let i = subjectWords.length - 1; i >= 0; i--) {
        const entry = this.processor?.resolveSayableWord?.(subjectWords[i], { allowRootFallback: true });
        if (entry) return entry.pos || 'Other';
      }
      return 'Other';
    }

    plan(promptText, analysis = null) {
      analysis = analysis || this.processor?.analyzePrompt?.(String(promptText || '')) || {};
      const firstWord = String(this._wordTokens(analysis)[0]?.lower || '').toLowerCase();
      const operatorRule = this.ruleStore?.operatorRule?.(firstWord) || null;
      const analyzedRole = analysis?.requestedSlot || analysis?.questionState?.requestedRole || 'content';
      const role = analyzedRole === 'content' && operatorRule?.default_role
        ? String(operatorRule.default_role)
        : analyzedRole;
      const family = analysis?.questionState?.family || operatorRule?.families?.[0] || (analysis?.isQuestion ? 'question' : 'statement');
      const subjectWords = this._subjectWords(analysis);
      const predicateWords = this._predicateWords(analysis);
      const objectWords = this._objectWords(analysis);
      const dependency = this._contextDependency(analysis, subjectWords);
      const complexity = this._complexity(analysis);
      const lexicalCoverage = clamp(analysis?.lexicalCoverage ?? 0);
      const expectedPOS = this._expectedPOS(analysis);
      const focusSeeds = this._focusSeeds(analysis, subjectWords, predicateWords, objectWords, role);
      const definitionHints = this.ruleStore?.definitionEntries?.(subjectWords, this.options.maxSubjectWords) || [];
      const candidateBudget = this._candidateBudget(complexity, role);
      const hasStructure = Boolean(expectedPOS.length || analysis?.responseScaffold?.words?.length || analysis?.clauseState);
      const fastEligible = Boolean(
        focusSeeds.length &&
        hasStructure &&
        lexicalCoverage >= this.options.lexicalFastFloor
      );
      ...
      const plan = {
        ...
        operatorRule: operatorRule ? {
          operator: firstWord,
          defaultRole: operatorRule.default_role || null,
          families: Array.isArray(operatorRule.families) ? operatorRule.families.slice(0, 8) : []
        } : null,
        definitionHints,
        responseWords: (analysis?.responseScaffold?.words || []).slice(0, 24),
        responseSource: analysis?.responseScaffold?.source || null,
        lexicalCoverage,
        complexity,
        candidateBudget,
        evidenceLimit: Math.max(48, Math.min(160, 56 + candidateBudget * 12)),
        anchorPOS: this._anchorPOS(subjectWords),
        fastEligible,
        selfContained: dependency.selfContained,
        requiresContext: dependency.requiresContext,
        contextReason: dependency.reason,
        contextStats: dependency,
        search: {
          initialHops: complexity < 0.45 ? 1 : 2,
          recoveryHops: complexity < 0.60 ? 2 : 3,
          preferFreshTrajectory: complexity > 0.55,
          targetConfidence: 80
        }
      };

      this.planCount++;
      if (fastEligible) this.fastEligibleCount++;
      this.last = plan;
      this.history.push({
        ...
        complexity,
        candidateBudget,
        seedCount: focusSeeds.length
      });
      if (this.history.length > this.options.planHistory) this.history.splice(0, this.history.length - this.options.planHistory);
      return plan;
    }

    status() {
      return {
        role: 'non-factual-fast-query-planner',
        externalRuleData: this.ruleStore?.status?.() || null,
        planCount: this.planCount,
        fastEligibleCount: this.fastEligibleCount,
        fastEligibleRate: this.planCount ? this.fastEligibleCount / this.planCount : 0,
        last: this.last ? {
          intent: this.last.intent,
          requestedRole: this.last.requestedRole,
          questionFamily: this.last.questionFamily,
          fastEligible: this.last.fastEligible,
          selfContained: this.last.selfContained,
          lexicalCoverage: this.last.lexicalCoverage,
          complexity: this.last.complexity,
          candidateBudget: this.last.candidateBudget,
          focusSeeds: this.last.focusSeeds.slice(0, 12)
        } : null
      };
    }
  }

  globalThis.VilotQueryPlanner = VilotQueryPlanner;
})();

/**
 * QueryPlanner production research extension.
 *
...
(() => {
  ...
  class VilotQueryPlannerProductionLab {
    constructor(options = {}) {
    ...
    status() {
      return {
        ...
        role: 'query-planning-production-research-extension',
        calls: this.calls,
  ...
  globalThis.VilotQueryPlannerProductionLab = VilotQueryPlannerProductionLab;
})();
