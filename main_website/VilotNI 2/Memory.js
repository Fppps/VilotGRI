 ...
 * VilotNI 2 - Memory.js
 *
 * Production working/episodic memory layer. Memory remembers experiences; it does not
 * try to be the language model. Correlation.js consumes these episodes and
 * consolidates their structural lessons into RSL.
 */
(() => {
  'use strict';
  ...
  class VilotMemory {
    constructor(options = {}) {
      this.options = {
        workingCapacity: Math.max(16, options.memoryWorkingCapacity | 0 || 48),
        episodicCapacity: Math.max(32, options.memoryEpisodicCapacity | 0 || 192),
        maxTextChars: Math.max(128, options.memoryMaxTextChars | 0 || 4096)
      };
      this.working = [];
      this.episodic = [];
      this.replayCursor = 0;
    ...
    _boundedPush(target, value, capacity) {
      target.push(value);
      if (target.length > capacity) target.splice(0, target.length - capacity);
    }

    remember(text, metadata = {}) {
      const clean = String(text || '').slice(0, this.options.maxTextChars).trim();
      ...
      this._boundedPush(this.working, episode, this.options.workingCapacity);
      if (metadata.episodic !== false) {
        this._boundedPush(this.episodic, episode, this.options.episodicCapacity);
      }
    ...
    nextReplay() {
      const source = this.episodic.length ? this.episodic : this.working;
      if (!source.length) return null;
      const index = this.replayCursor % source.length;
      this.replayCursor = (this.replayCursor + 1) >>> 0;
      return source[index] || null;
    }

    recent(limit = 16) {
      const n = Math.max(1, Math.min(64, limit | 0 || 16));
      return this.working.slice(-n);
    }

    reset() {
      this.working.length = 0;
      this.episodic.length = 0;
      this.replayCursor = 0;
      this.sequence = 0;
      this.totalRemembered = 0;
    }
    ...
    status() {
      return {
        role: 'working-and-episodic-context-only',
        workingEntries: this.working.length,
        episodicEntries: this.episodic.length,
  ...
  globalThis.VilotMemory = VilotMemory;
})();

/**
 * Memory production research extension.
 *
...
(() => {
  ...
  class VilotMemoryProductionLab {
    constructor(options = {}) {
    ...
    top64(items, score = x => Number(x?.score ?? x) || 0) {
      return this.topK(items, 64, score);
    }

    clampVariant1(x, lo = 0, hi = 1) {
      const n = Number(x);
      if (!Number.isFinite(n)) return lo;
      return Math.max(lo, Math.min(hi, n));
    }
  ...
  globalThis.VilotMemoryProductionLab = VilotMemoryProductionLab;
})();
